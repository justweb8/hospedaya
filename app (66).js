        ...
        <div style="background:white;border:1px solid var(--gris-borde);border-radius:14px;overflow:hidden;">
          <div style="padding:1rem 1.25rem;border-bottom:1px solid var(--gris-borde);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem;">
            ...
            <div style="display:flex;align-items:center;gap:0.5rem;">
              ...
              <div style="display:flex;align-items:center;gap:0.4rem;background:white;border:1px solid var(--gris-borde);border-radius:9px;padding:0.45rem 0.75rem;font-size:0.8rem;color:var(--texto-sub);">
                <select id="caja-filtro-tipo" onchange="filtrarMovimientos()" style="border:none;background:none;outline:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
...
function filtrarMovimientos() {
  const q    = (document.getElementById('caja-buscar')?.value||'').toLowerCase().trim();
  const tipo = document.getElementById('caja-filtro-tipo')?.value || '';
  const data = window._cajaMov || [];
  ...
  const tbody = document.getElementById('caja-movs-tbody');
  if (tbody) tbody.innerHTML = filtrados.length
    ? renderFilasCaja(filtrados)
    : '<tr><td colspan="8" style="padding:2rem;text-align:center;color:var(--texto-sub);">Sin resultados</td></tr>';
}

function descargarReporteCaja() {
  const turno = SESSION.turnoActivo;
  if (!turno) { toast('Sin turno', 'No hay turno abierto', 'warn'); return; }
  const movs  = window._cajaMov || [];
  ...
  const filasMov = movs.map(m => {
    const hora = new Date(m.created_at).toLocaleString('es-PE',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});
    const esIng = m.tipo === 'ingreso';
    return `<tr>
      <td>${hora}</td>
      <td>${escapeHtml(m.concepto||'—')}</td>
      <td>${escapeHtml(m.metodo_pago||'—')}</td>
      <td style="text-align:center;"><span style="padding:2px 10px;border-radius:999px;font-size:11px;font-weight:700;background:${esIng?'#F0FDF4':'#FEF2F2'};color:${esIng?'#16A34A':'#DC2626'};">${esIng?'Ingreso':'Egreso'}</span></td>
      <td style="text-align:right;font-weight:700;color:${esIng?'#16A34A':'#DC2626'};">${esIng?'+':'-'}S/ ${Number(m.monto).toFixed(2)}</td>
    </tr>`;
  }).join('');
...
<head>
<meta charset="UTF-8">
<title>Reporte de Caja — ${hotel?.nombre_comercial||''}</title>
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:'Segoe UI',Arial,sans-serif; font-size:13px; color:#1E293B; background:#F8FAFC; padding:32px; }
  .page { max-width:720px; margin:0 auto; background:white; border-radius:12px; box-shadow:0 2px 20px rgba(0,0,0,0.08); overflow:hidden; }
  .header { background:linear-gradient(135deg,#1E3A8A,#2563EB); color:white; padding:28px 32px; }
  .header h1 { font-size:22px; font-weight:700; margin-bottom:4px; }
  .header p { font-size:12px; opacity:0.85; }
  .body { padding:28px 32px; }
  .seccion { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:#64748B; margin:22px 0 10px; }
  ...
  .metodos { display:grid; grid-template-columns:repeat(2,1fr); gap:8px; margin-bottom:8px; }
  .met-card { display:flex; justify-content:space-between; align-items:center; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:10px 14px; }
  .met-card .met-lbl { font-size:12px; color:#475569; }
  .met-card .met-val { font-size:14px; font-weight:700; color:#1E3A8A; }
  table { width:100%; border-collapse:collapse; font-size:12px; }
  th { background:#F1F5F9; color:#475569; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; padding:10px 12px; text-align:left; border-bottom:1px solid #E2E8F0; }
  ...
  .saldo-box { background:#EFF6FF; border:1.5px solid #BFDBFE; border-radius:10px; padding:16px 20px; display:flex; justify-content:space-between; align-items:center; margin-top:8px; }
  .saldo-box .lbl { font-size:13px; font-weight:600; color:#1E3A8A; }
  .saldo-box .val { font-size:24px; font-weight:800; color:#1E3A8A; }
  .footer { background:#F8FAFC; border-top:1px solid #E2E8F0; padding:16px 32px; display:flex; justify-content:space-between; align-items:center; font-size:11px; color:#94A3B8; }
  .btn { padding:10px 22px; border-radius:8px; border:none; cursor:pointer; font-size:13px; font-weight:600; }
  .btn-primary { background:#2563EB; color:white; margin-right:8px; }
...
<div class="page">
  <div class="header">
    ...
    <p>${escapeHtml(hotel?.nombre_comercial||'Hotel')} · Generado el ${ahoraStr}</p>
  </div>
  <div class="body">

    <div class="seccion">Información del turno</div>
    <div class="info-row"><span class="lbl">Cajero</span><span class="val">${escapeHtml(cajero)}</span></div>
    <div class="info-row"><span class="lbl">Apertura del turno</span><span class="val">${apertura}</span></div>
    <div class="info-row"><span class="lbl">Generado</span><span class="val">${ahoraStr}</span></div>
    <div class="info-row"><span class="lbl">Fondo inicial</span><span class="val">S/ ${Number(turno.fondo_inicial).toFixed(2)}</span></div>
    <div class="info-row"><span class="lbl">Total movimientos</span><span class="val">${movs.length}</span></div>

    <div class="seccion">Resumen</div>
    <div class="kpis">
      <div class="kpi">
        <div class="val" style="color:#16A34A;">S/ ${totalIngresos.toFixed(2)}</div>
        <div class="lbl">Total ingresos</div>
      </div>
      <div class="kpi">
        <div class="val" style="color:#DC2626;">S/ ${res.egresos.toFixed(2)}</div>
        <div class="lbl">Total egresos</div>
      </div>
      <div class="kpi">
        <div class="val" style="color:#2563EB;">S/ ${(totalIngresos - res.egresos).toFixed(2)}</div>
        <div class="lbl">Neto del turno</div>
      </div>
      <div class="kpi">
        <div class="val" style="color:#1E293B;">S/ ${Number(turno.fondo_inicial).toFixed(2)}</div>
    ...
    <table>
      <thead><tr>
        <th>Hora</th><th>Concepto</th><th>Método</th><th style="text-align:center;">Tipo</th><th style="text-align:right;">Monto</th>
      </tr></thead>
      <tbody>${filasMov || '<tr><td colspan="5" style="text-align:center;padding:1rem;color:#94A3B8;">Sin movimientos</td></tr>'}</tbody>
    </table>

  </div>
  <div class="footer">
    <span>HospedaYa · Sistema de Gestión Hotelera</span>
    <div class="no-print">
...
// ════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════
//  MÓDULO CALENDARIO — Rediseño completo
// ════════════════════════════════════════════════════════════
  ...
  try {
    ...
    const [{ data: habs }, { data: reservas }] = await Promise.all([
      ...
      db.from('estadias_reservas')
        .select('id,fecha_entrada,fecha_salida_prev,fecha_salida_real,estado,habitacion_id,adelanto_pagado,huespedes(nombres,apellidos),habitaciones(numero,tipos_habitacion(nombre))')
        .eq('hotel_id', SESSION.hotel.id)
    ...
    window._calVista    = window._calVista || 'mes';
    window._calFecha    = window._calFecha || new Date();
    window._calFiltroHab = window._calFiltroHab || 'todas';
...
function renderCalendario() {
  ...
  // Paleta de colores por estado (igual al mockup)
  const estadoColor = {
    reservada:  { bg:'#DBEAFE', text:'#1D4ED8', dot:'#2563EB',  label:'Confirmada' },
  ...
  // Paleta de colores por reserva (para vista mes — cada reserva tiene su color único)
  const PALETA_MES = [
  ...
  const huespMes  = new Set(resMes.map(r=>r.huespedes?.nombres)).size;
  const habOcup   = new Set(resMes.filter(r=>r.estado==='activa').map(r=>r.habitacion_id)).size;
  const pctOcup   = habs.length > 0 ? Math.round((habOcup/habs.length)*100) : 0;

  // Mini calendario derecho
  const miniCal = renderMiniCal(fecha, reservas);

  // Contar por estado para leyenda
  const contEstados = {};
  reservas.forEach(r => { contEstados[r.estado] = (contEstados[r.estado]||0)+1; });
  ...
  if (esMobile) {
    ...
    const diaSelLabel = diaSelFecha.toLocaleDateString('es-PE',{weekday:'long',day:'numeric',month:'long',year:'numeric'});

    // Calcular puntos por día (qué días tienen reservas y de qué color)
    const puntosXDia = {};
    reservas.forEach((r,i) => {
      const ent = new Date(r.fecha_entrada); ent.setHours(0,0,0,0);
      const sal = new Date(r.fecha_salida_prev||r.fecha_salida_real||r.fecha_entrada);
      sal.setHours(0,0,0,0);
    ...
    contenido().innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">
        <div style="width:40px;height:40px;border-radius:12px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
          <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:20px;height:20px;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
        </div>
        <div>
          <h1 style="font-size:1.1rem;font-weight:700;color:var(--texto);margin:0;">Calendario de Reservas</h1>
          <p style="font-size:0.7rem;color:var(--texto-sub);margin:0;">Solo se muestran las habitaciones con reservas</p>
        </div>
      </div>

      <!-- Navegación mes + toggle -->
      <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.9rem;">
        <button onclick="navCal(-1)" style="width:30px;height:30px;border:1.5px solid var(--gris-borde);border-radius:8px;background:white;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        ...
        <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:0.35rem;cursor:pointer;">
          <span style="font-size:0.95rem;font-weight:700;color:var(--texto);text-transform:capitalize;">${mesStr}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--texto-sub)" stroke-width="2" stroke-linecap="round" style="width:14px;height:14px;"><polyline points="6 9 12 15 18 9"/></svg>
        </div>
        <button onclick="navCal(1)" style="width:30px;height:30px;border:1.5px solid var(--gris-borde);border-radius:8px;background:white;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        ...
        <div style="display:flex;border:1.5px solid var(--gris-borde);border-radius:9px;overflow:hidden;flex-shrink:0;">
          ${[['dia','Día'],['semana','Semana'],['mes','Mes']].map(([v,l])=>`
            <button onclick="setCalVista('${v}')"
              style="padding:0.38rem 0.6rem;border:none;cursor:pointer;font-size:0.72rem;font-weight:600;white-space:nowrap;
                background:${vista===v?'var(--azul)':'white'};color:${vista===v?'white':'var(--texto-sub)'};
                border-left:${v!=='dia'?'1.5px solid var(--gris-borde)':'none'};">${l}</button>`).join('')}
        </div>
      </div>

      <!-- Stats 4 en fila compacta -->
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:0.5rem;margin-bottom:1rem;">
        ${[
          {svg:'<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>',val:resMes.length,sub:'Reservas',c:'#2563EB',bg:'#EFF6FF'},
          {svg:'<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>',val:huespMes,sub:'Huéspedes',c:'#16A34A',bg:'#F0FDF4'},
          {svg:'<path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/><path d="M6 8v9"/>',val:habOcup,sub:'Hab. ocupadas',c:'#CA8A04',bg:'#FEFCE8'},
          {svg:'<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>',val:pctOcup+'%',sub:'Ocupación',c:'#7C3AED',bg:'#F5F3FF'},
        ].map(s=>`
          <div style="background:${s.bg};border-radius:12px;padding:0.6rem 0.4rem;text-align:center;">
            <div style="display:flex;justify-content:center;margin-bottom:0.2rem;">
              <svg viewBox="0 0 24 24" fill="none" stroke="${s.c}" stroke-width="2" stroke-linecap="round" style="width:16px;height:16px;">${s.svg}</svg>
            </div>
            <div style="font-size:0.95rem;font-weight:800;color:${s.c};line-height:1;">${s.val}</div>
            <div style="font-size:0.58rem;color:${s.c};opacity:0.75;margin-top:0.1rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${s.sub}</div>
          </div>`).join('')}
      </div>

      <!-- Calendario tipo app nativa -->
      <div style="background:white;border-radius:16px;border:1.5px solid var(--gris-borde);margin-bottom:1rem;overflow:hidden;">
        <!-- Días de la semana -->
        <div style="display:grid;grid-template-columns:repeat(7,1fr);padding:0.5rem 0.25rem 0.35rem;border-bottom:1px solid var(--gris-borde);">
          ${['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'].map((d,i)=>`
            <div style="text-align:center;font-size:0.7rem;font-weight:700;color:${i===0||i===6?'#DC2626':'var(--texto-sub)'};">${d}</div>`).join('')}
        </div>
        <!-- Días del mes -->
        <div id="cal-grid-mobile" style="display:grid;grid-template-columns:repeat(7,1fr);padding:0.25rem;">
          ${renderDiasMobile(fecha, puntosXDia, diaSelStr)}
        </div>
      </div>
      ...
      <div>
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem;">
          <span style="font-size:0.88rem;font-weight:700;color:var(--texto);text-transform:capitalize;">${diaSelLabel}</span>
          <span style="font-size:0.78rem;font-weight:600;color:var(--texto-sub);">${resDiaSel.length} reserva${resDiaSel.length!==1?'s':''}</span>
        </div>

        ${resDiaSel.length===0
          ? `<div style="text-align:center;padding:2rem;background:white;border-radius:14px;border:1.5px solid var(--gris-borde);color:var(--texto-sub);">
              <div style="font-size:1.5rem;margin-bottom:0.35rem;">📅</div>
              <div style="font-size:0.82rem;font-weight:600;">Sin reservas este día</div>
            </div>`
          : resDiaSel.map(r => {
              const nom   = `${r.huespedes?.nombres||''} ${r.huespedes?.apellidos||''}`.trim() || 'Huésped';
              const hab   = r.habitaciones?.numero||'—';
              const tipo  = r.habitaciones?.tipos_habitacion?.nombre||'';
              const ent   = new Date(r.fecha_entrada);
              const sal   = new Date(r.fecha_salida_prev||r.fecha_salida_real||r.fecha_entrada);
              const noches= Math.max(1,Math.round((sal-ent)/(1000*60*60*24)));
              const fmtR  = d => d.toLocaleDateString('es-PE',{day:'numeric',month:'short',year:'numeric'});
              const dot   = {reservada:'#16A34A',activa:'#2563EB',check_out:'#CA8A04'}[r.estado]||'#94A3B8';
              const ec    = {
                reservada: {bg:'#F0FDF4', text:'#16A34A', label:'Confirmada'},
                activa:    {bg:'#EFF6FF', text:'#2563EB', label:'En estadía'},
                check_out: {bg:'#FEFCE8', text:'#CA8A04', label:'Por llegar'},
              }[r.estado]||{bg:'#F3F4F6',text:'#6B7280',label:r.estado};
              return `
              <div onclick="abrirReservaDetalle('${r.id}')" style="background:white;border:1.5px solid var(--gris-borde);border-radius:14px;padding:0.85rem 1rem;margin-bottom:0.65rem;display:flex;align-items:center;gap:0.85rem;cursor:pointer;-webkit-tap-highlight-color:transparent;" ontouchstart="this.style.background='var(--gris-bg)'" ontouchend="this.style.background='white'">
                <!-- Indicador + hab -->
                <div style="display:flex;flex-direction:column;align-items:center;gap:0.25rem;flex-shrink:0;">
                  <span style="width:12px;height:12px;border-radius:50%;background:${dot};"></span>
                  <span style="font-size:0.75rem;font-weight:800;color:var(--texto);">${escapeHtml(hab)}</span>
                  <span style="font-size:0.6rem;color:var(--texto-sub);text-align:center;">${escapeHtml(tipo)}</span>
                </div>
                <!-- Info -->
                <div style="flex:1;min-width:0;">
                  <div style="font-weight:700;font-size:0.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(nom)}</div>
                  <div style="font-size:0.72rem;color:var(--texto-sub);margin-top:0.1rem;">${fmtR(ent)} - ${fmtR(sal)} (${noches} noche${noches!==1?'s':''})</div>
                  <div style="font-size:0.72rem;color:var(--texto-sub);">${r.num_huespedes||1} huésped${(r.num_huespedes||1)!==1?'es':''}</div>
                </div>
                <!-- Badge + flecha -->
                <div style="display:flex;align-items:center;gap:0.4rem;flex-shrink:0;">
                  <span style="font-size:0.65rem;font-weight:700;color:${ec.text};background:${ec.bg};padding:0.25rem 0.6rem;border-radius:999px;white-space:nowrap;">${ec.label}</span>
                  <svg viewBox="0 0 24 24" fill="none" stroke="#CBD5E1" stroke-width="2" stroke-linecap="round" style="width:14px;height:14px;"><polyline points="9 18 15 12 9 6"/></svg>
                </div>
              </div>`;
            }).join('')}
        ...
        ${resDiaSel.length > 0 ? `
        <button onclick="navegarA('reservas')" style="width:100%;display:flex;align-items:center;justify-content:center;gap:0.5rem;background:white;border:1.5px solid var(--gris-borde);border-radius:12px;padding:0.75rem;font-size:0.82rem;font-weight:600;color:var(--azul);cursor:pointer;margin-bottom:4.5rem;">
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2" stroke-linecap="round" style="width:14px;height:14px;"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/></svg>
          Ver todas las reservas del día
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2.5" stroke-linecap="round" style="width:12px;height:12px;"><polyline points="9 18 15 12 9 6"/></svg>
        </button>` : ''}
      </div>
  ...
  contenido().innerHTML = `
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:1.5rem;">
      <div style="display:flex;align-items:center;gap:1rem;">
        <div style="width:52px;height:52px;border-radius:14px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
          <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:26px;height:26px;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
        </div>
        <div>
          <h1 style="font-size:1.55rem;font-weight:700;color:var(--texto);margin:0;">Calendario de Reservas</h1>
          <p style="font-size:0.83rem;color:var(--texto-sub);margin:0.2rem 0 0;">Visualiza y gestiona todas tus reservas por mes</p>
        </div>
      </div>
      <button onclick="abrirNuevaReserva()" style="display:flex;align-items:center;gap:0.5rem;padding:0.7rem 1.25rem;background:linear-gradient(135deg,var(--azul),#2563EB);color:white;border:none;border-radius:10px;font-size:0.88rem;font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(37,99,235,0.3);">
        <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" style="width:16px;height:16px;"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
        + Nueva reserva
      </button>
    </div>

    <!-- Navegación mes + toggle vistas -->
    <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:1.25rem;">
      <div style="display:flex;align-items:center;gap:0.75rem;">
        <button onclick="irHoyCal()" style="padding:0.5rem 1rem;background:white;border:1.5px solid var(--gris-borde);border-radius:10px;font-size:0.85rem;font-weight:600;color:var(--texto);cursor:pointer;">Hoy</button>
        <div style="display:flex;gap:0.35rem;">
        ...
        <h2 style="font-size:1.25rem;font-weight:700;color:var(--texto);margin:0;text-transform:capitalize;">${mesStr}
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:16px;height:16px;vertical-align:middle;margin-left:4px;cursor:pointer;" onclick=""><polyline points="6 9 12 15 18 9"/></svg>
        </h2>
      </div>
      <!-- Toggle Día/Semana/Mes -->
      <div style="display:flex;border:1.5px solid var(--gris-borde);border-radius:10px;overflow:hidden;background:white;">
        ${[['dia','Día'],['semana','Semana'],['mes','Mes']].map(([v,l])=>`
          <button onclick="setCalVista('${v}')"
            style="padding:0.5rem 1.1rem;border:none;cursor:pointer;font-size:0.85rem;font-weight:600;white-space:nowrap;
              background:${vista===v?'var(--azul)':'white'};color:${vista===v?'white':'var(--texto-sub)'};
              border-left:${v!=='dia'?'1.5px solid var(--gris-borde)':'none'};">
            ${l}
          </button>`).join('')}
      </div>
    </div>
    ...
    <!-- Stats 4 en fila -->
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.25rem;" class="cocina-metrics-grid">
      ${calStatDesktop('<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>','Reservas este mes',   resMes.length, '#2563EB','#EFF6FF')}
      ${calStatDesktop('<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>','Huéspedes',          huespMes,       '#16A34A','#F0FDF4')}
      ${calStatDesktop('<path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/><path d="M6 8v9"/>','Habitaciones ocupadas',habOcup,       '#CA8A04','#FEFCE8')}
      ${calStatDesktop('<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>','Ocupación promedio',  pctOcup+'%',    '#7C3AED','#F5F3FF')}
    </div>

    <!-- Layout: Calendario + Panel derecho -->
    <div style="display:grid;grid-template-columns:1fr 280px;gap:1.25rem;align-items:start;" class="cal-layout">

      <!-- Calendario principal -->
      <div style="background:white;border:1.5px solid var(--gris-borde);border-radius:16px;overflow:hidden;">
        <!-- Filtro habitaciones -->
        <div style="display:flex;align-items:center;justify-content:space-between;padding:0.85rem 1.1rem;border-bottom:1.5px solid var(--gris-borde);">
          <div style="display:flex;gap:0.5rem;overflow-x:auto;scrollbar-width:none;">
            <button onclick="setCalFiltroHab('todas')" style="padding:0.35rem 0.85rem;border-radius:999px;font-size:0.78rem;font-weight:600;cursor:pointer;white-space:nowrap;
      ...
      <div style="display:flex;flex-direction:column;gap:1rem;">
        <!-- Mini calendario -->
        <div style="background:white;border:1.5px solid var(--gris-borde);border-radius:16px;padding:1.1rem;">
          ${miniCal}
        </div>
        ...
        <div style="background:white;border:1.5px solid var(--gris-borde);border-radius:16px;padding:1.1rem;">
          ${Object.entries(estadoColor).map(([k,v])=>`
            <div style="display:flex;align-items:center;gap:0.65rem;padding:0.4rem 0;${k!=='cancelada'?'border-bottom:1px solid var(--gris-borde);':''}">
              <span style="width:12px;height:12px;border-radius:50%;background:${v.dot};flex-shrink:0;"></span>
              <span style="font-size:0.82rem;color:var(--texto);flex:1;">${v.label}</span>
              <span style="font-size:0.82rem;font-weight:700;color:var(--texto);">${contEstados[k]||0}</span>
            </div>`).join('')}
        </div>

        <!-- Sincroniza -->
        <div style="background:#EFF6FF;border:1.5px solid #BFDBFE;border-radius:16px;padding:1.1rem;text-align:center;">
...
// ── GRID MENSUAL ─────────────────────────────────────────────
function renderDiasMobile(fecha, puntosXDia, diaSelStr) {
  ...
  return celdas.map(d=>{
    ...
    return `<div onclick="selDiaCal('${dStr}')" style="display:flex;flex-direction:column;align-items:center;padding:0.3rem 0;cursor:pointer;-webkit-tap-highlight-color:transparent;">
      <div style="width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;
        background:${esHoy?'var(--azul)':esSel?'#EFF6FF':'transparent'};
        border:${esSel&&!esHoy?'2px solid var(--azul)':'2px solid transparent'};
        font-size:0.82rem;font-weight:${esHoy||esSel?'800':'${esMesCur?400:300}'};
        color:${esHoy?'white':esMesCur?(esFin?'#DC2626':'var(--texto)'):'#CBD5E1'};">
        ${d.getDate()}
      </div>
      <!-- Puntos de reservas -->
      <div style="display:flex;gap:2px;margin-top:2px;min-height:8px;">
        ${puntos.slice(0,3).map(c=>`<span style="width:6px;height:6px;border-radius:50%;background:${c};display:inline-block;"></span>`).join('')}
      </div>
    </div>`;
  }).join('');
}

function selDiaCal(dStr) {
  window._calDiaSeleccionado = dStr;
  renderCalendario();
}

function renderMesGrid(reservas, fecha, PALETA, esMobile, estadoColor) {
  const año = fecha.getFullYear(), mes = fecha.getMonth();
  const primerDia = new Date(año, mes, 1);
  const ultDia    = new Date(año, mes+1, 0);
  const hoy = new Date(); hoy.setHours(0,0,0,0);

  // Empezar en domingo (como el mockup)
  let inicio = new Date(primerDia);
  inicio.setDate(1 - primerDia.getDay());
  ...
  // Mapear colores a reservas únicas
  const resColorMap = {};
  reservas.forEach((r,i) => { resColorMap[r.id] = PALETA[i % PALETA.length]; });

  const DIAS = ['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'];
  const celH = esMobile ? 'min-height:52px' : 'min-height:90px';
  const fs   = esMobile ? '0.62' : '0.78';
  const fsDia= esMobile ? '0.78' : '0.9';
  const fsEv = esMobile ? '0.58' : '0.72';
  const maxEv= esMobile ? 1 : 3;

  return `
    <table style="width:100%;border-collapse:collapse;">
      <thead>
        <tr style="background:#F8FAFC;">
          ${DIAS.map((d,i)=>`<th style="padding:${esMobile?'0.4':'0.65'}rem 0.25rem;text-align:center;font-size:${fs}rem;font-weight:700;color:${i===0||i===6?'#DC2626':'var(--texto-sub)'};border-bottom:1.5px solid var(--gris-borde);">${esMobile?d[0]:d}</th>`).join('')}
        </tr>
      </thead>
      <tbody>
        ${Array.from({length:Math.ceil(celdas.length/7)},(_,si)=>{
          const sem = celdas.slice(si*7, si*7+7);
          return `<tr>
            ${sem.map((d,di)=>{
              const esMesCur = d.getMonth()===mes;
              ...
              return `<td style="vertical-align:top;padding:${esMobile?'3':'5'}px;border:1px solid var(--gris-borde);${celH};background:${esHoy2?'#EFF6FF':esFin&&esMesCur?'#FAFAFA':'white'};width:${100/7}%;">
                <!-- Número del día -->
                <div style="display:flex;justify-content:center;margin-bottom:${esMobile?'2':'4'}px;">
                  <span style="${esHoy2?`width:${esMobile?'22':'26'}px;height:${esMobile?'22':'26'}px;background:var(--azul);color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;`:''}font-size:${fsDia}rem;font-weight:${esHoy2?'800':esMesCur?'500':'400'};color:${esHoy2?'white':esMesCur?(esFin?'#DC2626':'var(--texto)'):'#CBD5E1'};">${d.getDate()}</span>
                </div>
                <!-- Eventos -->
                ${esMesCur ? resDia.slice(0,maxEv).map(r=>{
                  const c = resColorMap[r.id] || PALETA[0];
                  const nom = `${r.huespedes?.nombres||''}`.trim() || 'Huésped';
                  const hab = r.habitaciones?.numero||'';
                  const tipo= r.habitaciones?.tipos_habitacion?.nombre||'';
                  return `<div onclick="abrirReservaDetalle('${r.id}')" style="background:${c.bg};color:${c.text};font-size:${fsEv}rem;font-weight:600;padding:${esMobile?'1px 3px':'2px 6px'};border-radius:4px;margin-bottom:2px;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border-left:3px solid ${c.text};">
                    <div style="overflow:hidden;text-overflow:ellipsis;">${esMobile?'●':escapeHtml(nom)}</div>
                    ${!esMobile&&hab?`<div style="font-size:0.62rem;opacity:0.8;">${escapeHtml(hab)}${tipo?' - '+escapeHtml(tipo):''}</div>`:''}
                  </div>`;
                }).join('') : ''}
                ${esMesCur && resDia.length>maxEv ? `<div onclick="verDiaReservas('${dStr}')" style="font-size:${esMobile?'0.55':'0.65'}rem;color:var(--azul);font-weight:700;cursor:pointer;text-align:center;">+${resDia.length-maxEv} más</div>` : ''}
              </td>`;
            }).join('')}
          </tr>`;
        }).join('')}
...
// ── GRID SEMANAL ─────────────────────────────────────────────
function renderSemanaGrid(habs, reservas, fecha, PALETA) {
  const hoy = new Date(); hoy.setHours(0,0,0,0);
  const lunes = new Date(fecha);
  lunes.setDate(fecha.getDate() - fecha.getDay()); // Semana empieza Dom
  ...
  return `
    <table style="width:100%;border-collapse:collapse;">
      <thead>
        <tr style="background:#F8FAFC;">
          <th style="width:100px;padding:0.6rem 0.75rem;font-size:0.75rem;font-weight:700;color:var(--texto-sub);border-right:2px solid var(--gris-borde);border-bottom:1.5px solid var(--gris-borde);text-align:left;">HAB.</th>
          ${dias.map((d,i)=>{
            const esHoy2=d.toDateString()===hoy.toDateString();
            const esFin=i===0||i===6;
            return `<th style="padding:0.6rem 0.25rem;text-align:center;font-size:0.78rem;border-bottom:1.5px solid var(--gris-borde);border-right:1px solid var(--gris-borde);background:${esHoy2?'#EFF6FF':esFin?'#FEF2F2':'transparent'};">
              <div style="font-weight:600;color:${esHoy2?'var(--azul)':esFin?'#DC2626':'var(--texto-sub)'};">${DIAS[i]}</div>
              <div style="font-size:1rem;font-weight:${esHoy2?'800':'600'};color:${esHoy2?'var(--azul)':'var(--texto)'};">${d.getDate()}</div>
            </th>`;
          }).join('')}
        </tr>
      </thead>
        ...
        ${habs.map(h=>`<tr style="border-bottom:1px solid var(--gris-borde);">
          ...
          ${dias.map(d=>{
            ...
            return `<td style="padding:4px;height:50px;border-right:1px solid var(--gris-borde);background:${esHoy2?'#EFF6FF':esFin?'#FAFAFA':'white'};">
              ${res?`<div onclick="abrirReservaDetalle('${res.id}')" style="background:${c.bg};color:${c.text};border-left:3px solid ${c.text};border-radius:5px;padding:3px 6px;font-size:0.72rem;font-weight:700;cursor:pointer;height:100%;display:flex;flex-direction:column;justify-content:center;overflow:hidden;">${escapeHtml(nom.split(' ')[0])}</div>`:''}
            </td>`;
          }).join('')}
        </tr>`).join('')}
      </tbody>
...
function renderDiaGrid(habs, reservas, fecha) {
  ...
  return `
    <div style="padding:1.25rem;">
      <div style="font-weight:700;font-size:1rem;margin-bottom:1rem;text-transform:capitalize;">${dLabel}</div>
      ${resDia.length===0
        ? `<div style="text-align:center;padding:2.5rem;color:var(--texto-sub);">
            <div style="font-size:2rem;margin-bottom:0.5rem;">📅</div>
            <div style="font-weight:600;">Sin reservas este día</div>
           </div>`
        : resDia.map(r=>{
            const nom=`${r.huespedes?.nombres||''} ${r.huespedes?.apellidos||''}`.trim();
            const hab=r.habitaciones?.numero||'—';
            const tipo=r.habitaciones?.tipos_habitacion?.nombre||'';
            const ec={reservada:{bg:'#DBEAFE',text:'#1D4ED8'},activa:{bg:'#D1FAE5',text:'#065F46'},check_out:{bg:'#F5F3FF',text:'#5B21B6'}}[r.estado]||{bg:'#F3F4F6',text:'#6B7280'};
            return `<div onclick="abrirReservaDetalle('${r.id}')" style="display:flex;align-items:center;gap:1rem;background:${ec.bg};border-left:4px solid ${ec.text};border-radius:10px;padding:0.85rem 1rem;margin-bottom:0.65rem;cursor:pointer;">
              <div style="flex:1;">
                <div style="font-weight:700;font-size:0.9rem;color:${ec.text};">${escapeHtml(nom)}</div>
                <div style="font-size:0.75rem;color:var(--texto-sub);margin-top:0.15rem;">Hab. ${escapeHtml(hab)} ${tipo?'· '+escapeHtml(tipo):''}</div>
              </div>
              <span style="font-size:0.72rem;font-weight:700;color:${ec.text};background:white;padding:0.2rem 0.65rem;border-radius:999px;">${r.estado}</span>
            </div>`;
          }).join('')}
    </div>`;
}
...
// ── MINI CALENDARIO ──────────────────────────────────────────
function renderMiniCal(fecha, reservas) {
  const año=fecha.getFullYear(), mes=fecha.getMonth();
  const hoy=new Date(); hoy.setHours(0,0,0,0);
  const primerDia=new Date(año,mes,1);
  let inicio=new Date(primerDia); inicio.setDate(1-primerDia.getDay());
  const celdas=[]; let cur=new Date(inicio);
  ...
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.85rem;">
      <button onclick="navCal(-1)" style="background:none;border:none;cursor:pointer;color:var(--texto-sub);padding:0.2rem;">‹</button>
      <span style="font-size:0.85rem;font-weight:700;text-transform:capitalize;">${mesStr}</span>
      <button onclick="navCal(1)" style="background:none;border:none;cursor:pointer;color:var(--texto-sub);padding:0.2rem;">›</button>
    </div>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr>${DIAS.map(d=>`<th style="font-size:0.65rem;font-weight:700;color:var(--texto-sub);text-align:center;padding:0.2rem;">${d}</th>`).join('')}</tr></thead>
      <tbody>
        ${Array.from({length:6},(_,si)=>`<tr>${celdas.slice(si*7,si*7+7).map(d=>{
          ...
          return `<td style="text-align:center;padding:0.2rem;cursor:pointer;" onclick="window._calFecha=new Date('${dStr}');window._calVista='dia';renderCalendario();">
            <div style="width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto;
              ...
              color:${esHoy2?'white':esMesCur?(tieneRes?'var(--azul)':'var(--texto)'):'#CBD5E1'};">
              ${d.getDate()}
            </div>
          </td>`;
        }).join('')}</tr>`).join('')}
...
// ── HELPERS ──────────────────────────────────────────────────
function calStatNuevo(ico, label, valor, color, bg) {
  return `<div class="card" style="padding:0.85rem;min-width:0;">
    <div style="font-size:1.2rem;margin-bottom:0.35rem;">${ico}</div>
    <div style="font-size:1.4rem;font-weight:800;color:${color};line-height:1;">${valor}</div>
    <div style="font-size:0.68rem;color:var(--texto-sub);margin-top:0.15rem;">${label}</div>
  </div>`;
}
function calStatDesktop(svgPath, label, valor, color, bg) {
  return `<div class="card" style="padding:1.1rem;display:flex;align-items:center;gap:0.85rem;">
    <div style="width:48px;height:48px;border-radius:13px;background:${bg};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
      <svg viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" style="width:24px;height:24px;">${svgPath}</svg>
    </div>
    <div>
      <div style="font-size:1.6rem;font-weight:800;color:var(--texto);line-height:1;">${valor}</div>
      <div style="font-size:0.76rem;color:var(--texto-sub);margin-top:0.15rem;">${label}</div>
    </div>
  </div>`;
}
...
function setCalFiltroHab(id) {
  window._calFiltroHab = id;
  renderCalendario();
}
function navCal(dir) {
  const f=window._calFecha; const v=window._calVista;
  if (v==='semana'||v==='dia') f.setDate(f.getDate()+dir*(v==='dia'?1:7));
...
function abrirReservaDetalle(id) {
  ...
  if (!res) return;
  const nom=`${res.huespedes?.nombres||''} ${res.huespedes?.apellidos||''}`.trim();
  const hab=res.habitaciones?.numero||'—';
  ...
  abrirModal('Detalle de reserva',`
    <div style="display:flex;flex-direction:column;gap:0.85rem;">
      <div style="display:flex;align-items:center;gap:0.85rem;">
        <div style="width:48px;height:48px;border-radius:13px;background:${ec.bg};color:${ec.text};font-weight:800;font-size:1rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
          ${nom.split(/\s+/).slice(0,2).map(p=>p[0]||'').join('').toUpperCase()||'?'}
        </div>
        <div>
          <div style="font-weight:700;font-size:1.05rem;">${escapeHtml(nom)}</div>
          <span style="font-size:0.72rem;font-weight:700;color:${ec.text};background:${ec.bg};padding:0.2rem 0.65rem;border-radius:999px;">${ec.label}</span>
        </div>
      </div>
      <div style="background:var(--gris-bg);border-radius:12px;padding:1rem;display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
        <div><div style="font-size:0.7rem;color:var(--texto-sub);">Habitación</div><div style="font-weight:700;">Hab. ${escapeHtml(hab)} ${tipo?'· '+escapeHtml(tipo):''}</div></div>
        <div><div style="font-size:0.7rem;color:var(--texto-sub);">Adelanto</div><div style="font-weight:700;color:#16A34A;">${soles(res.adelanto_pagado||0)}</div></div>
        <div><div style="font-size:0.7rem;color:var(--texto-sub);">Check-in</div><div style="font-weight:700;">${ent}</div></div>
        <div><div style="font-size:0.7rem;color:var(--texto-sub);">Check-out</div><div style="font-weight:700;">${sal}</div></div>
      </div>
      <div style="display:flex;gap:0.5rem;">
        <button onclick="cerrarModal();navegarA('reservas')" style="${ST.btnSec};flex:1;padding:0.75rem;">Ver en reservas</button>
        ${res.estado==='reservada'?`<button onclick="cerrarModal();hacerCheckinReserva('${res.id}')" style="${ST.btnPri};flex:1;padding:0.75rem;">✅ Check-in</button>`:''}
      </div>
    </div>
  `);
}
  ...
  try {
    ...
    if (esMobile) {
      ...
      contenido().innerHTML = `
        ...
        <div style="display:flex;gap:0.5rem;overflow-x:auto;scrollbar-width:none;margin-bottom:1rem;padding-bottom:0.1rem;">
          <div style="display:flex;align-items:center;gap:0.4rem;background:white;border:1px solid var(--gris-borde);border-radius:999px;padding:0.45rem 0.85rem;font-size:0.78rem;color:var(--texto-sub);white-space:nowrap;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:13px;height:13px;flex-shrink:0;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            <select id="res-filtro-fecha-m" onchange="filtrarReservasMobile()" style="border:none;background:none;outline:none;font-size:0.78rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
    ...
    contenido().innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:0.65rem;flex-wrap:wrap;margin-bottom:0;">
        ...
        <!-- Filtro fechas -->
        <div style="display:flex;align-items:center;gap:0.45rem;background:white;border:1px solid var(--gris-borde);border-radius:10px;padding:0.45rem 0.75rem;font-size:0.83rem;color:var(--texto-sub);">
          <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;flex-shrink:0;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
          <select id="res-filtro-fecha" onchange="filtrarReservas()" style="border:none;background:none;outline:none;font-size:0.83rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
            <option value="">Todas las fechas</option>
            <option value="hoy">Hoy</option>
            <option value="manana">Mañana</option>
            <option value="semana">Esta semana</option>
      ...
      <div style="background:white;border:1px solid var(--gris-borde);border-radius:12px;padding:1.25rem 1.5rem;display:flex;align-items:center;gap:1.25rem;flex-wrap:wrap;position:relative;overflow:hidden;">
        ...
        <button onclick="navegarA('calendario')" style="display:flex;align-items:center;gap:0.5rem;background:white;border:1.5px solid var(--azul);border-radius:10px;padding:0.6rem 1.1rem;font-size:0.83rem;font-weight:600;color:var(--azul);cursor:pointer;position:relative;white-space:nowrap;">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
          Ver calendario
        </button>
      </div>
    ...
    return `
      <div class="card" style="padding:0.9rem 1rem;margin-bottom:0.65rem;">
        ...
        <div style="display:flex;align-items:center;gap:0.65rem;padding:0.6rem 0;border-top:1px solid var(--gris-borde);border-bottom:1px solid var(--gris-borde);">
          ...
          <button onclick="abrirOpcionesReserva('${r.id}','${r.habitacion_id}')" style="padding:0.3rem 0.5rem;background:white;border:1px solid var(--gris-borde);border-radius:8px;cursor:pointer;color:var(--texto-sub);font-size:1rem;line-height:1;">···</button>
        </div>
...
function filtrarReservasMobile() {
  const q       = (document.getElementById('res-buscar-m')?.value || document.getElementById('res-buscar')?.value || '').toLowerCase().trim();
  const filtFec = document.getElementById('res-filtro-fecha-m')?.value || '';
  const filtEst = document.getElementById('res-filtro-estado-m')?.value || '';
  ...
  const filtradas = data.filter(r => {
    ...
    if (filtEst && fn) {
      ...
      if (filtEst === 'hoy') {
        const entrada = new Date(r.fecha_entrada); entrada.setHours(0,0,0,0);
        if (entrada.getTime() !== hoy.getTime()) return false;
      } else if (!labelLow.includes(filtEst)) return false;
    }
  ...
  cont.innerHTML = filtradas.length
    ? renderTarjetasReservasMobile(filtradas, fn)
...
function filtrarReservas() {
  ...
  const filtFec = document.getElementById('res-filtro-fecha')?.value || '';
  const filtEst = document.getElementById('res-filtro-estado')?.value || '';
  const data    = window._reservasData || [];
  const fn      = window._reservasEstadoFn;
  ...
  if (tbody) {
    tbody.innerHTML = filtradas.length
      ? renderFilasReservas(filtradas, fn)
      : `<tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--texto-sub);">Sin resultados</td></tr>`;
  }
}

function renderFilasReservas(reservas, estadoReserva) {
  const tdCss = () => 'padding:0.85rem 1rem;border-bottom:1px solid var(--gris-borde);vertical-align:middle;';
  return reservas.map(r => {
    const h = r.huespedes || {};
    const hab = r.habitaciones || {};
    const nombre = `${h.nombres||''} ${h.apellidos||''}`.trim() || 'Sin nombre';
    const iniciales = nombre.split(/\s+/).slice(0,2).map(p=>p[0]||'').join('').toUpperCase()||'?';
    const [estLabel, estColor, estBg] = estadoReserva(r);
    return `<tr class="res-fila" data-buscar="${escapeHtml((nombre+' '+(hab.numero||'')+' '+(r.fecha_entrada||'')).toLowerCase())}">
      <td style="${tdCss()}">${fechaHora(r.fecha_entrada)}</td>
      <td style="${tdCss()}">${fechaHora(r.fecha_salida_prev)}</td>
      <td style="${tdCss()}"><span style="font-weight:700;">${escapeHtml(hab.numero||'—')}</span></td>
      <td style="${tdCss()}">
        <div style="display:flex;align-items:center;gap:0.65rem;">
          <div style="width:34px;height:34px;border-radius:9px;background:rgba(37,99,235,0.1);color:var(--azul);font-weight:700;font-size:0.78rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${iniciales}</div>
          <div>
      ...
      <td style="${tdCss()}"><span style="font-weight:700;color:${Number(r.adelanto_pagado)>0?'var(--verde)':'var(--texto-sub)'};">${soles(r.adelanto_pagado)}</span></td>
      <td style="${tdCss()}">
        <span style="font-size:0.72rem;font-weight:700;color:${estColor};background:${estBg};padding:0.25rem 0.75rem;border-radius:999px;display:inline-flex;align-items:center;gap:0.3rem;">
          <span style="width:6px;height:6px;border-radius:50%;background:${estColor};"></span>
          ${estLabel}
...
function abrirOpcionesReserva(reservaId, habId) {
  ...
  const nombre = `${h.nombres||''} ${h.apellidos||''}`.trim() || 'Sin nombre';
  const numHab = hab.numero || '—';
  const entrada = reserva?.fecha_entrada ? new Date(reserva.fecha_entrada).toLocaleDateString('es-PE', {day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}) : '—';

  abrirModal('Opciones de reserva', `
    <div style="margin-bottom:1rem;padding:0.85rem;background:var(--gris-bg);border-radius:10px;display:flex;flex-direction:column;gap:0.25rem;">
      <div style="font-weight:700;font-size:0.92rem;color:var(--texto);">${escapeHtml(nombre)}</div>
      <div style="font-size:0.78rem;color:var(--texto-sub);">Hab. ${escapeHtml(numHab)} · Entrada: ${entrada}</div>
    </div>
    <div style="display:flex;flex-direction:column;gap:0.5rem;">
      <button style="${ST.btnPri};width:100%;padding:0.75rem;display:flex;align-items:center;justify-content:center;gap:0.5rem;" onclick="cerrarModal();confirmarLlegadaReserva('${reservaId}','${habId}')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
        Hacer Check-in
      </button>
      <button style="${ST.btnSec};width:100%;padding:0.75rem;display:flex;align-items:center;justify-content:center;gap:0.5rem;" onclick="cerrarModal();verHuespedDesdeReserva('${reservaId}')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        Ver ficha del huésped
      </button>
      <button style="${ST.btnSec};width:100%;padding:0.75rem;display:flex;align-items:center;justify-content:center;gap:0.5rem;" onclick="cerrarModal();editarFechasReserva('${reservaId}')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
        Editar fechas
      </button>
      <button style="${ST.btnDanger};width:100%;padding:0.75rem;display:flex;align-items:center;justify-content:center;gap:0.5rem;" onclick="cancelarReserva('${reservaId}','${habId}')">
...
async function editarFechasReserva(reservaId) {
  const reserva = (window._reservasData || []).find(r => r.id === reservaId);
  if (!reserva) { toast('Error', 'Reserva no encontrada', 'error'); return; }
  ...
  abrirModal('Editar fechas de reserva', `
    <div style="${ST.grupo}">
      <label style="${ST.label}">Fecha y hora de entrada *</label>
      <input style="${ST.input}" id="edit-entrada" type="datetime-local" value="${toLocal(reserva.fecha_entrada)}" required>
    </div>
    <div style="${ST.grupo}">
      <label style="${ST.label}">Fecha y hora de salida prevista *</label>
      <input style="${ST.input}" id="edit-salida" type="datetime-local" value="${toLocal(reserva.fecha_salida_prev)}" required>
    </div>
    <button onclick="guardarFechasReserva('${reservaId}')" style="${ST.btnPri};width:100%;padding:0.75rem;margin-top:0.5rem;">Guardar cambios</button>
  `, { ancho: '400px' });
}

async function guardarFechasReserva(reservaId) {
    ...
    contenido().innerHTML = `
      ...
      <div style="background:white;border:1px solid var(--gris-borde);border-radius:12px;padding:0.85rem 1.1rem;display:flex;align-items:center;gap:0.65rem;flex-wrap:wrap;margin-bottom:1rem;">
        ...
        <div style="display:flex;align-items:center;gap:0.4rem;background:var(--gris-bg);border:1px solid var(--gris-borde);border-radius:9px;padding:0.45rem 0.8rem;font-size:0.8rem;color:var(--texto-sub);">
          ...
          <select id="hues-filtro-fecha" onchange="filtrarHuespedes()" style="border:none;background:none;outline:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
        ...
        <div style="display:flex;align-items:center;gap:0.4rem;background:var(--gris-bg);border:1px solid var(--gris-borde);border-radius:9px;padding:0.45rem 0.8rem;font-size:0.8rem;color:var(--texto-sub);">
          <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:14px;height:14px;flex-shrink:0;"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
          <select id="hues-filtro-estado" onchange="filtrarHuespedes()" style="border:none;background:none;outline:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
        ...
        <div style="display:flex;align-items:center;gap:0.4rem;background:var(--gris-bg);border:1px solid var(--gris-borde);border-radius:9px;padding:0.45rem 0.8rem;font-size:0.8rem;color:var(--texto-sub);">
          <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:14px;height:14px;flex-shrink:0;"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/></svg>
          <select id="hues-filtro-orden" onchange="filtrarHuespedes()" style="border:none;background:none;outline:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
            <option value="reciente">Más recientes</option>
            <option value="antiguo">Más antiguos</option>
            <option value="nombre">A → Z</option>
          </select>
        </div>
        <button onclick="limpiarFiltrosHuespedes()" style="background:none;border:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;padding:0.5rem 0.6rem;border-radius:9px;white-space:nowrap;" onmouseover="this.style.background='var(--gris-bg)'" onmouseout="this.style.background='none'">
          Limpiar filtros
        </button>
...
function limpiarFiltrosHuespedes() {
  const ids = ['hues-buscar','hues-filtro-fecha','hues-filtro-estado'];
  ids.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  const orden = document.getElementById('hues-filtro-orden');
  ...
  try {
    ...
    contenido().innerHTML = `
        ...
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;align-items:center;">
          ...
          <div style="display:flex;align-items:center;gap:0.5rem;background:white;border:1px solid var(--gris-borde);border-radius:10px;padding:0.5rem 0.9rem;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;flex-shrink:0;"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input type="text" id="tienda-buscar-top" placeholder="Buscar producto…" oninput="filtrarTiendaBuscar()" style="border:none;background:none;outline:none;font-size:0.83rem;width:160px;font-family:inherit;color:var(--texto);">
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;margin-bottom:1rem;">
        ...
        <div style="display:flex;align-items:center;gap:0.5rem;">
          ...
          <div style="display:flex;align-items:center;gap:0.4rem;background:white;border:1px solid var(--gris-borde);border-radius:10px;padding:0.45rem 0.8rem;font-size:0.8rem;color:var(--texto-sub);">
            <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:13px;height:13px;flex-shrink:0;"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/></svg>
            <select id="tienda-orden" onchange="filtrarTiendaBuscar()" style="border:none;background:none;outline:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
          ...
          <div style="display:flex;align-items:center;gap:0.4rem;background:white;border:1px solid var(--gris-borde);border-radius:10px;padding:0.45rem 0.8rem;font-size:0.8rem;color:var(--texto-sub);">
            <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:13px;height:13px;flex-shrink:0;"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
            <select id="tienda-filtro-estado" onchange="filtrarTiendaBuscar()" style="border:none;background:none;outline:none;font-size:0.8rem;color:var(--texto-sub);cursor:pointer;font-family:inherit;">
              <option value="">Filtros: Todos</option>
      ...
      <div style="background:white;border:1px solid var(--gris-borde);border-radius:12px;padding:1.25rem 1.5rem;display:flex;align-items:center;gap:1.25rem;flex-wrap:wrap;position:relative;overflow:hidden;">
        ...
        <button onclick="verReporteStockPDF()" style="display:flex;align-items:center;gap:0.5rem;background:white;border:1.5px solid var(--azul);border-radius:10px;padding:0.6rem 1.1rem;font-size:0.83rem;font-weight:600;color:var(--azul);cursor:pointer;position:relative;white-space:nowrap;">
...
function verReporteStockPDF() {
  const prods = window._productosCache || [];
  const hotel = SESSION.hotel;
  const ahora = new Date().toLocaleString('es-PE',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'});
  const stockTotal  = prods.reduce((s,p) => s + Number(p.stock_actual||0), 0);
  const valorCosto  = prods.reduce((s,p) => s + Number(p.stock_actual||0)*Number(p.costo_compra||0), 0);
  const valorVenta  = prods.reduce((s,p) => s + Number(p.stock_actual||0)*Number(p.precio_venta||0), 0);
  const sinStock    = prods.filter(p => Number(p.stock_actual||0) <= 0).length;
  const bajoStock   = prods.filter(p => Number(p.stock_actual||0) > 0 && Number(p.stock_actual||0) <= Number(p.stock_minimo||0)).length;

  const filas = [...prods].sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||'','es')).map((p,i) => {
    const stock = Number(p.stock_actual||0);
    const min   = Number(p.stock_minimo||0);
    const estado = stock <= 0 ? ['Sin stock','#DC2626','#FEF2F2'] : stock <= min ? ['Stock bajo','#EA580C','#FFF7ED'] : ['En stock','#16A34A','#F0FDF4'];
    return `<tr style="border-bottom:1px solid #F1F5F9;">
      <td style="padding:8px 12px;color:#64748B;font-size:12px;">${i+1}</td>
      <td style="padding:8px 12px;font-weight:600;font-size:12px;">${escapeHtml(p.nombre||'—')}</td>
      <td style="padding:8px 12px;font-size:12px;"><span style="background:#F1F5F9;border-radius:6px;padding:2px 8px;font-size:11px;">${escapeHtml(p.categoria||'general')}</span></td>
      <td style="padding:8px 12px;text-align:right;font-size:12px;">S/ ${Number(p.precio_venta||0).toFixed(2)}</td>
      <td style="padding:8px 12px;text-align:right;font-size:12px;color:#64748B;">S/ ${Number(p.costo_compra||0).toFixed(2)}</td>
      <td style="padding:8px 12px;text-align:right;font-weight:700;font-size:13px;color:${stock<=0?'#DC2626':stock<=min?'#EA580C':'#1E293B'};">${stock}</td>
      <td style="padding:8px 12px;text-align:right;font-size:12px;">S/ ${(stock*Number(p.precio_venta||0)).toFixed(2)}</td>
      <td style="padding:8px 12px;text-align:center;"><span style="padding:2px 10px;border-radius:999px;font-size:11px;font-weight:700;background:${estado[2]};color:${estado[1]};">${estado[0]}</span></td>
    </tr>`;
  }).join('');
...
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  body{font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#1E293B;background:#F8FAFC;padding:32px;}
  .page{max-width:860px;margin:0 auto;background:white;border-radius:12px;box-shadow:0 2px 20px rgba(0,0,0,0.08);overflow:hidden;}
  .header{background:linear-gradient(135deg,#4C1D95,#7C3AED);color:white;padding:28px 32px;}
  .header h1{font-size:22px;font-weight:700;margin-bottom:4px;}
  .header p{font-size:12px;opacity:0.85;}
  ...
  .footer{background:#F8FAFC;border-top:1px solid #E2E8F0;padding:14px 32px;display:flex;justify-content:space-between;align-items:center;font-size:11px;color:#94A3B8;}
  .btn{padding:9px 20px;border-radius:8px;border:none;cursor:pointer;font-size:13px;font-weight:600;}
  .btn-p{background:#7C3AED;color:white;margin-right:8px;}
  .btn-s{background:#F1F5F9;color:#475569;border:1px solid #E2E8F0;}
  @media print{body{background:white;padding:0;}.page{box-shadow:none;border-radius:0;}.no-print{display:none!important;}}
</style></head><body>
<div class="page">
  <div class="header">
    <h1>Reporte de Stock</h1>
    <p>${escapeHtml(hotel?.nombre_comercial||'Hotel')} · Generado el ${ahora}</p>
  </div>
  <div class="body">
    <div class="kpis">
      <div class="kpi"><div class="val" style="color:#7C3AED;">${prods.length}</div><div class="lbl">Productos</div></div>
      <div class="kpi"><div class="val" style="color:#2563EB;">${stockTotal}</div><div class="lbl">Unidades totales</div></div>
      <div class="kpi"><div class="val" style="color:#16A34A;">S/ ${valorVenta.toFixed(2)}</div><div class="lbl">Valor venta</div></div>
      <div class="kpi"><div class="val" style="color:#EA580C;">${bajoStock}</div><div class="lbl">Stock bajo</div></div>
      <div class="kpi"><div class="val" style="color:#DC2626;">${sinStock}</div><div class="lbl">Sin stock</div></div>
    </div>
    <table>
      <thead><tr>
        <th style="width:36px;">#</th><th>Producto</th><th>Categoría</th>
        <th class="r">Precio venta</th><th class="r">Costo</th>
        <th class="r">Stock</th><th class="r">Valor en stock</th><th class="c">Estado</th>
      </tr></thead>
      <tbody>${filas||'<tr><td colspan="8" style="text-align:center;padding:2rem;color:#94A3B8;">Sin productos</td></tr>'}</tbody>
    </table>
    <div style="margin-top:12px;padding:12px 0;border-top:2px solid #E2E8F0;display:flex;justify-content:flex-end;gap:24px;font-size:12px;">
      <span>Valor total (costo): <strong>S/ ${valorCosto.toFixed(2)}</strong></span>
      <span>Valor total potencial (venta): <strong>S/ ${valorVenta.toFixed(2)}</strong></span>
    </div>
  </div>
  <div class="footer">
    <span>HospedaYa · Sistema de Gestión Hotelera</span>
    <div class="no-print">
      ...
      <button class="btn btn-s" onclick="window.close()">Cerrar</button>
    </div>
  </div>
</div></body></html>`);
  ventana.document.close();
...
function filtrarTiendaBuscar() {
  const q      = (document.getElementById('tienda-buscar')?.value || document.getElementById('tienda-buscar-top')?.value || '').toLowerCase().trim();
  const orden  = document.getElementById('tienda-orden')?.value || 'nombre';
  const estado = document.getElementById('tienda-filtro-estado')?.value || '';
  const cat    = window._tiendaFiltrocat || 'Todos';
  let lista    = [...(window._productosCache || [])];

  // Filtro categoría
  if (cat !== 'Todos') lista = lista.filter(p => (p.categoria||'general') === cat);

  // Filtro búsqueda texto
  if (q) lista = lista.filter(p => `${p.nombre||''} ${p.categoria||''}`.toLowerCase().includes(q));

  // Filtro estado stock
  if (estado === 'en_stock')   lista = lista.filter(p => Number(p.stock_actual||0) > Number(p.stock_minimo||0));
  if (estado === 'bajo_stock') lista = lista.filter(p => Number(p.stock_actual||0) > 0 && Number(p.stock_actual||0) <= Number(p.stock_minimo||0));
  if (estado === 'sin_stock')  lista = lista.filter(p => Number(p.stock_actual||0) <= 0);

  // Ordenar
  if (orden === 'nombre')       lista.sort((a,b) => (a.nombre||'').localeCompare(b.nombre||'', 'es'));
  if (orden === 'nombre_desc')  lista.sort((a,b) => (b.nombre||'').localeCompare(a.nombre||'', 'es'));
  if (orden === 'stock_asc')    lista.sort((a,b) => Number(a.stock_actual||0) - Number(b.stock_actual||0));
  if (orden === 'stock_desc')   lista.sort((a,b) => Number(b.stock_actual||0) - Number(a.stock_actual||0));
  if (orden === 'precio_asc')   lista.sort((a,b) => Number(a.precio_venta||0) - Number(b.precio_venta||0));
  if (orden === 'precio_desc')  lista.sort((a,b) => Number(b.precio_venta||0) - Number(a.precio_venta||0));

  const tbody = document.getElementById('tienda-tbody');
  if (tbody) tbody.innerHTML = lista.length
    ? renderFilasTienda(lista, 'Todos') // pasamos Todos para no doble-filtrar por cat
    : '<tr><td colspan="10" style="text-align:center;padding:2rem;color:var(--texto-sub);">Sin resultados</td></tr>';
}
    ...
    if (esMobile) {
      contenido().innerHTML = `
        ...
        <div style="display:flex;align-items:flex-start;gap:0.85rem;margin-bottom:1rem;">
          ...
          <div>
            <h1 style="font-size:1.15rem;font-weight:700;color:var(--texto);margin:0;">Configuración de Habitaciones</h1>
            <p style="font-size:0.7rem;color:var(--texto-sub);margin:0.1rem 0 0;">Crea tus tipos con tarifas y luego gestiona tus habitaciones s</p>
          </div>
        </div>

        <!-- 2 accesos rápidos tipo tarjeta -->
        <div style="display:flex;flex-direction:column;gap:0.6rem;margin-bottom:1.25rem;">
          <div onclick="document.getElementById('seccion-tipos-mobile').scrollIntoView({behavior:'smooth'})"
            style="background:white;border:1.5px solid var(--gris-borde);border-radius:14px;padding:1rem;display:flex;align-items:center;gap:0.85rem;cursor:pointer;"
            ontouchstart="this.style.background='var(--gris-bg)'" ontouchend="this.style.background='white'">
            <div style="width:40px;height:40px;border-radius:11px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:20px;height:20px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
            </div>
            <div style="flex:1;">
              <div style="font-weight:700;font-size:0.9rem;">Tipos de habitación</div>
              <div style="font-size:0.72rem;color:var(--texto-sub);">Define categorías, capacidades y tarifas.</div>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:16px;height:16px;flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
          <div onclick="document.getElementById('seccion-habs-mobile').scrollIntoView({behavior:'smooth'})"
            style="background:white;border:1.5px solid var(--gris-borde);border-radius:14px;padding:1rem;display:flex;align-items:center;gap:0.85rem;cursor:pointer;"
            ontouchstart="this.style.background='var(--gris-bg)'" ontouchend="this.style.background='white'">
            <div style="width:40px;height:40px;border-radius:11px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:20px;height:20px;"><path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/><path d="M6 8v9"/></svg>
            </div>
            <div style="flex:1;">
              <div style="font-weight:700;font-size:0.9rem;">Habitaciones</div>
              <div style="font-size:0.72rem;color:var(--texto-sub);">Administra tus habitaciones y su estado.</div>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:16px;height:16px;flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
        </div>
        ...
        <!-- ── TIPOS DE HABITACIÓN ── -->
        <div id="seccion-tipos-mobile">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem;">
            <div>
              <div style="font-weight:700;font-size:1rem;">Tipos de habitación</div>
              <div style="font-size:0.72rem;color:var(--texto-sub);">${(tipos||[]).length} tipo${(tipos||[]).length!==1?'s':''} registrado${(tipos||[]).length!==1?'s':''}</div>
            </div>
            <button onclick="abrirFormTipo()" style="display:flex;align-items:center;gap:0.35rem;padding:0.55rem 0.9rem;background:var(--azul);color:white;border:none;border-radius:10px;font-size:0.78rem;font-weight:700;cursor:pointer;box-shadow:0 4px 12px rgba(37,99,235,0.3);">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" style="width:14px;height:14px;"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Nuevo tipo
            </button>
          </div>
          ...
          ${(tipos||[]).length===0
            ? `<div class="card" style="text-align:center;padding:2rem;color:var(--texto-sub);">
                <div style="font-size:2rem;margin-bottom:0.5rem;">🛏️</div>
                <div style="font-weight:600;">Sin tipos aún</div>
                <div style="font-size:0.8rem;margin-top:0.25rem;">Crea tu primer tipo de habitación</div>
               </div>`
            : tipos.map((t,i) => `
              <div style="background:white;border:1px solid var(--gris-borde);border-radius:14px;padding:0.9rem 1rem;margin-bottom:0.6rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.6rem;">
                  <div style="display:flex;align-items:center;gap:0.65rem;">
                    <span style="font-size:0.85rem;font-weight:700;color:var(--texto-sub);width:16px;">${i+1}</span>
                    <div>
                      <div style="font-weight:700;font-size:0.92rem;">${escapeHtml(t.nombre)}</div>
                      <div style="font-size:0.7rem;color:var(--texto-sub);display:flex;align-items:center;gap:0.3rem;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
                        ${t.capacidad_max} persona${t.capacidad_max!==1?'s':''}
                      </div>
                    </div>
                  </div>
                  <button onclick="abrirFormTipo('${t.id}')" style="padding:0.3rem 0.5rem;background:white;border:1px solid var(--gris-borde);border-radius:8px;cursor:pointer;color:var(--texto-sub);font-size:0.9rem;line-height:1;">⋮</button>
                </div>
                <!-- Tarifas + estado -->
                <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
                  <span style="display:flex;align-items:center;gap:0.35rem;background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:0.35rem 0.7rem;font-size:0.78rem;font-weight:700;color:#16A34A;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2" stroke-linecap="round" style="width:13px;height:13px;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    ${soles(t.tarifa_noche)}
                  </span>
                  <span style="font-size:0.78rem;color:var(--texto-sub);">${soles(t.tarifa_horas)} / ${t.horas_bloque}h</span>
                  <span style="margin-left:auto;font-size:0.7rem;font-weight:700;color:#16A34A;background:#F0FDF4;padding:0.2rem 0.65rem;border-radius:999px;display:flex;align-items:center;gap:0.25rem;">
                    <span style="width:5px;height:5px;border-radius:50%;background:#16A34A;"></span>Activo
                  </span>
                </div>
              </div>`).join('')}
          ...
          <!-- Ver todos los tipos -->
          ${(tipos||[]).length > 3 ? `
          <div style="background:white;border:1.5px solid var(--gris-borde);border-radius:12px;padding:0.85rem 1rem;display:flex;align-items:center;justify-content:center;gap:0.5rem;cursor:pointer;margin-top:0.25rem;margin-bottom:1.25rem;" onclick="abrirFormTipo()">
            <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/></svg>
            <span style="font-size:0.83rem;font-weight:600;color:var(--azul);">Ver todos los tipos</span>
            <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2.5" stroke-linecap="round" style="width:13px;height:13px;"><polyline points="9 18 15 12 9 6"/></svg>
          </div>` : '<div style="margin-bottom:1.25rem;"></div>'}
        </div>

        <!-- ── HABITACIONES ── -->
        <div id="seccion-habs-mobile">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem;">
            <div>
              <div style="font-weight:700;font-size:1rem;">Habitaciones</div>
              <div style="font-size:0.72rem;color:var(--texto-sub);">${(habs||[]).length} habitación(es) registrada(s)</div>
            </div>
            <button onclick="abrirFormHabitacion()" ${(tipos||[]).length===0?'disabled':''} style="display:flex;align-items:center;gap:0.35rem;padding:0.55rem 0.9rem;background:${(tipos||[]).length===0?'#94A3B8':'var(--azul)'};color:white;border:none;border-radius:10px;font-size:0.78rem;font-weight:700;cursor:pointer;">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" style="width:14px;height:14px;"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Nueva hab.
            </button>
          </div>
          ...
          ${(habs||[]).length===0
            ? `<div class="card" style="text-align:center;padding:2rem;color:var(--texto-sub);">
                <div style="font-size:2rem;margin-bottom:0.5rem;">🛏️</div>
                <div style="font-weight:600;">Sin habitaciones aún</div>
                <div style="font-size:0.8rem;margin-top:0.25rem;">${(tipos||[]).length===0?'Crea un tipo primero':'Agrega tu primera habitación'}</div>
               </div>`
            : habs.map((h,i) => {
                const ec = ESTADO_CFG[h.estado] || { dot:'#64748B', bg:'#F1F5F9', label: h.estado };
                return `
                <div style="background:white;border:1px solid var(--gris-borde);border-radius:14px;padding:0.9rem 1rem;margin-bottom:0.6rem;display:flex;align-items:center;gap:0.75rem;">
                  <span style="font-size:0.85rem;font-weight:700;color:var(--texto-sub);width:16px;flex-shrink:0;">${i+1}</span>
                  <div style="width:42px;height:42px;border-radius:11px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-weight:800;font-size:0.9rem;color:var(--azul);">${escapeHtml(h.numero)}</div>
                  <div style="flex:1;min-width:0;">
                    <div style="font-weight:700;font-size:0.88rem;">Hab. ${escapeHtml(h.numero)}</div>
                    <div style="font-size:0.7rem;color:var(--texto-sub);">${escapeHtml(h.tipos_habitacion?.nombre||'—')} · Piso ${escapeHtml(h.piso||'—')}</div>
                  </div>
                  <span style="font-size:0.68rem;font-weight:700;color:${ec.dot};background:${ec.bg};padding:0.2rem 0.6rem;border-radius:999px;flex-shrink:0;">${ec.label}</span>
                  <button onclick="abrirFormHabitacion('${h.id}')" style="padding:0.3rem 0.5rem;background:white;border:1px solid var(--gris-borde);border-radius:8px;cursor:pointer;color:var(--texto-sub);font-size:0.9rem;line-height:1;flex-shrink:0;">⋮</button>
                </div>`;
              }).join('')}

          <!-- Ver todos -->
          <div style="background:white;border:1.5px solid var(--gris-borde);border-radius:12px;padding:0.85rem 1rem;display:flex;align-items:center;justify-content:center;gap:0.5rem;cursor:pointer;margin-top:0.25rem;" onclick="navegarA('rack')">
            <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/></svg>
            <span style="font-size:0.83rem;font-weight:600;color:var(--azul);">Ver todos los tipos</span>
            <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2.5" stroke-linecap="round" style="width:13px;height:13px;"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
...
async function moduloPersonal() {
  ...
  try {
    ...
    const esMobile = window.innerWidth <= 768;
    window._personalCache = lista;
    window._personalRolColor = ROL_COLOR;
    ...
    if (esMobile) {
      contenido().innerHTML = `
        ...
        <div style="display:flex;align-items:center;gap:0.85rem;margin-bottom:1rem;">
          <div style="width:44px;height:44px;border-radius:12px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:22px;height:22px;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
          </div>
          <div>
            <h1 style="font-size:1.2rem;font-weight:700;color:var(--texto);margin:0;">Personal</h1>
            <p style="font-size:0.7rem;color:var(--texto-sub);margin:0;">Gestiona los usuarios del sistema y sus permisos</p>
          </div>
        </div>

        <!-- Botón crear empleado ancho completo -->
        <button onclick="abrirCrearEmpleado()" style="width:100%;padding:0.85rem;background:linear-gradient(135deg,var(--azul),#2563EB);color:white;border:none;border-radius:14px;font-size:0.95rem;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:0.5rem;box-shadow:0 4px 16px rgba(37,99,235,0.3);margin-bottom:1rem;">
          <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" style="width:18px;height:18px;"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          Crear empleado
        </button>
        ...
        <!-- 4 KPIs en 2×2 -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;margin-bottom:1rem;">
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.4rem;">
              <div style="width:32px;height:32px;border-radius:9px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;">
                <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:16px;height:16px;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
              </div>
              <span style="font-size:0.65rem;font-weight:700;color:#16A34A;background:#F0FDF4;padding:0.15rem 0.4rem;border-radius:999px;">↑ 0%</span>
            </div>
            <div style="font-size:1.5rem;font-weight:700;color:var(--texto);line-height:1;">${totalUsuarios}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);margin-top:0.15rem;">Total de usuarios</div>
          </div>
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.4rem;">
              <span style="width:14px;height:14px;border-radius:50%;background:#16A34A;box-shadow:0 0 0 3px #BBF7D0;"></span>
              <span style="font-size:0.65rem;font-weight:700;color:#16A34A;background:#F0FDF4;padding:0.15rem 0.4rem;border-radius:999px;">↑ 100%</span>
            </div>
            <div style="font-size:1.5rem;font-weight:700;color:var(--texto);line-height:1;">${activos}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);margin-top:0.15rem;">Usuarios activos</div>
          </div>
          <div class="card" style="padding:0.85rem;min-width:0;background:#FEF2F2;border-color:#FECACA;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.4rem;">
              <span style="width:14px;height:14px;border-radius:50%;background:#DC2626;box-shadow:0 0 0 3px #FECACA;"></span>
              <span style="font-size:0.65rem;font-weight:700;color:#DC2626;background:white;padding:0.15rem 0.4rem;border-radius:999px;">↓ 0%</span>
            </div>
            <div style="font-size:1.5rem;font-weight:700;color:var(--texto);line-height:1;">${inactivos}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);margin-top:0.15rem;">Usuarios inactivos</div>
          </div>
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="width:32px;height:32px;border-radius:9px;background:#F5F3FF;display:flex;align-items:center;justify-content:center;margin-bottom:0.4rem;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#7C3AED" stroke-width="2" stroke-linecap="round" style="width:16px;height:16px;"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
            </div>
            <div style="font-size:1.5rem;font-weight:700;color:var(--texto);line-height:1;">${roles}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);margin-top:0.15rem;">Roles asignados</div>
          </div>
        </div>
        ...
        <div style="display:flex;gap:0.5rem;margin-bottom:1rem;">
          <div style="flex:1;position:relative;">
            ...
            <input type="text" id="pers-buscar" placeholder="Buscar por nombre, rol o email…"
              oninput="filtrarPersonalMobile(this.value)"
              style="width:100%;padding:0.65rem 0.75rem 0.65rem 2.35rem;border:1.5px solid var(--gris-borde);border-radius:12px;font-size:0.83rem;background:white;box-sizing:border-box;outline:none;font-family:inherit;"
              onfocus="this.style.borderColor='var(--azul)'" onblur="this.style.borderColor='var(--gris-borde)'">
          </div>
        ...
        <!-- Lista -->
        <div id="pers-cards-mobile">
          ${renderPersonalCardsMobile(lista, ROL_COLOR)}
        </div>

        <!-- Paginación -->
        <div style="display:flex;align-items:center;justify-content:space-between;margin:0.75rem 0;font-size:0.75rem;color:var(--texto-sub);">
          <span>Mostrando 1 de ${totalUsuarios} usuarios</span>
          <div style="display:flex;gap:0.35rem;">
            <button style="width:28px;height:28px;border:1px solid var(--gris-borde);border-radius:7px;background:white;cursor:pointer;display:flex;align-items:center;justify-content:center;">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
        ...
        <!-- Banner inferior -->
        <div style="background:white;border:1px solid var(--gris-borde);border-radius:14px;padding:1rem;display:flex;align-items:center;gap:0.85rem;position:relative;overflow:hidden;">
          <svg style="position:absolute;bottom:0;right:0;opacity:0.07;" viewBox="0 0 200 80" width="200" height="80" preserveAspectRatio="none">
            <path d="M0 50 Q50 10 100 40 Q150 70 200 30 L200 80 L0 80 Z" fill="#2563EB"/>
          </svg>
          <div style="width:38px;height:38px;border-radius:10px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;position:relative;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          </div>
          <div style="flex:1;position:relative;">
            <div style="font-weight:700;font-size:0.85rem;color:var(--texto);">Gestiona los accesos de tu equipo</div>
            <div style="font-size:0.7rem;color:var(--texto-sub);margin-top:0.1rem;">Asigna roles, controla permisos y mantén un registro de la actividad de cada usuario.</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2.5" stroke-linecap="round" style="width:16px;height:16px;flex-shrink:0;position:relative;"><polyline points="9 18 15 12 9 6"/></svg>
        </div>
      `;
...
function renderPersonalCardsMobile(lista, ROL_COLOR) {
  if (!lista.length) return `
    <div style="text-align:center;padding:2.5rem;color:var(--texto-sub);">
      <div style="font-size:2.5rem;margin-bottom:0.75rem;">👤</div>
      <div style="font-weight:600;color:var(--texto);">Sin empleados aún</div>
    </div>`;
  ...
  return lista.map(p => {
    ...
    const fechaAcceso = ultimoAcceso
      ? new Date(ultimoAcceso).toLocaleDateString('es-PE',{day:'numeric',month:'short',year:'numeric'}) + ' - ' +
        new Date(ultimoAcceso).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'})
      : 'Sin accesos';
      ...
      <div class="card" style="padding:0.9rem 1rem;margin-bottom:0.6rem;">
        ...
        <div style="display:flex;align-items:flex-start;gap:0.75rem;margin-bottom:0.65rem;">
          ...
          <div style="display:flex;align-items:center;gap:0.4rem;flex-shrink:0;">
            ...
            <span style="font-size:0.65rem;font-weight:700;color:${activo?'#16A34A':'#DC2626'};background:${activo?'#F0FDF4':'#FEF2F2'};padding:0.2rem 0.6rem;border-radius:999px;display:flex;align-items:center;gap:0.2rem;">
              <span style="width:5px;height:5px;border-radius:50%;background:${activo?'#16A34A':'#DC2626'};"></span>
              ${activo?'Activo':'Inactivo'}
            </span>
            <button onclick="abrirOpcionesEmpleado('${p.user_id}','${escapeHtml(nombre)}')" style="padding:0.25rem 0.35rem;background:white;border:1px solid var(--gris-borde);border-radius:7px;cursor:pointer;color:var(--texto-sub);font-size:1rem;line-height:1;">⋮</button>
          </div>
        </div>
        <!-- Último acceso -->
...
function filtrarPersonalMobile(q) {
  const cont = document.getElementById('pers-cards-mobile');
  if (!cont) return;
...
function abrirOpcionesEmpleado(userId, nombre) {
  abrirModal('Opciones: ' + nombre, `
    <div style="display:flex;flex-direction:column;gap:0.5rem;">
      <button onclick="cerrarModal();abrirResetPass('${userId}','${escapeHtml(nombre)}')"
        style="${ST.btnSec};display:flex;align-items:center;gap:0.75rem;justify-content:flex-start;padding:0.85rem 1rem;width:100%;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="width:17px;height:17px;"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
        Resetear contraseña
      </button>
    </div>
  `, { ancho:'320px' });
  ...
  try {
    ...
    if (esMobile) {
      contenido().innerHTML = `
        ...
        <div style="display:flex;align-items:center;gap:0.85rem;margin-bottom:1rem;">
          <div style="width:44px;height:44px;border-radius:12px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:22px;height:22px;"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
          </div>
          <div>
            <h1 style="font-size:1.2rem;font-weight:700;color:var(--texto);margin:0;">Mi Suscripción</h1>
            <p style="font-size:0.7rem;color:var(--texto-sub);margin:0;">Gestiona tu plan y mantén tu hotel siempre en marcha</p>
          </div>
        </div>

        <!-- Card plan actual -->
        <div class="card" style="padding:1.25rem;margin-bottom:0.75rem;">
          <div style="font-size:0.68rem;font-weight:700;color:var(--texto-sub);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:0.5rem;">PLAN ACTUAL</div>
        ...
        <div class="card" style="padding:1.5rem;text-align:center;margin-bottom:0.75rem;">
          <div style="position:relative;width:160px;height:160px;margin:0 auto 1rem;">
            <svg viewBox="0 0 200 200" width="160" height="160">
              <circle cx="100" cy="100" r="80" fill="none" stroke="#E7EBF2" stroke-width="16" stroke-linecap="round"/>
              <circle cx="100" cy="100" r="80" fill="none" stroke="${estadoColor}" stroke-width="16" stroke-linecap="round"
                stroke-dasharray="${circ}" stroke-dashoffset="${offset}"
                transform="rotate(-90 100 100)" style="transition:stroke-dashoffset 1s ease;"/>
            </svg>
            <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
              <div style="font-size:2.75rem;font-weight:800;color:${estadoColor};line-height:1;">${diasMostrar}</div>
              <div style="font-size:0.72rem;color:var(--texto-sub);font-weight:500;">días restantes</div>
            </div>
          </div>
        ...
        <!-- Aviso renovación -->
        <div style="display:flex;align-items:center;gap:0.75rem;background:#F0FDF4;border:1px solid #BBF7D0;border-radius:12px;padding:0.85rem 1rem;margin-bottom:0.75rem;">
          <div style="width:36px;height:36px;border-radius:10px;background:#BBF7D0;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
          </div>
          <div style="font-size:0.78rem;color:#16A34A;font-weight:600;">Renueva a tiempo y evita interrupciones en el servicio.</div>
        </div>

        <!-- Banner Haz crecer tu hotel -->
        <div style="background:white;border:1px solid var(--gris-borde);border-radius:14px;padding:1rem;display:flex;align-items:center;gap:0.85rem;position:relative;overflow:hidden;margin-bottom:0.75rem;">
          <svg style="position:absolute;bottom:0;right:0;opacity:0.07;" viewBox="0 0 200 80" width="200" height="80" preserveAspectRatio="none">
            <path d="M0 50 Q50 10 100 40 Q150 70 200 30 L200 80 L0 80 Z" fill="#2563EB"/>
          </svg>
          <div style="width:38px;height:38px;border-radius:10px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;position:relative;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><path d="M2 4l3 12h14l3-12-6 7.5-4-6-4 6L2 4z"/></svg>
          </div>
          <div style="flex:1;position:relative;">
            <div style="font-weight:700;font-size:0.88rem;color:var(--texto);">Haz crecer tu hotel</div>
            <div style="font-size:0.72rem;color:var(--texto-sub);margin-top:0.1rem;">Optimiza tu operación con más herramientas y funcionalidades.</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2.5" stroke-linecap="round" style="width:16px;height:16px;flex-shrink:0;position:relative;"><polyline points="9 18 15 12 9 6"/></svg>
        </div>
        ...
        <div class="card" style="padding:1.1rem;margin-bottom:1rem;">
          <div style="font-weight:700;font-size:0.95rem;margin-bottom:0.85rem;">Beneficios de tu plan</div>
          ${beneficios.map((b,i)=>`
            <div style="display:flex;align-items:center;gap:0.65rem;padding:0.45rem 0;${i<beneficios.length-1?'border-bottom:1px solid var(--gris-borde);':''}">
              ...
              <span style="font-size:0.83rem;color:var(--texto);">${b}</span>
            </div>`).join('')}
        </div>
      `;
      return;
...
async function moduloReportes() {
  ...
  const periodos = ['Hoy','Últimos 7 días','Últimos 30 días','Este mes'];
  const esMobile = window.innerWidth <= 768;

  const fmtFechaCorta = d => {
    const dd = d.toLocaleDateString('es-PE',{day:'2-digit',month:'2-digit',year:'numeric'});
    return dd;
  };
  ...
  if (esMobile) {
    contenido().innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:0.85rem;margin-bottom:1rem;">
        <div style="width:44px;height:44px;border-radius:12px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
          <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:22px;height:22px;"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/></svg>
        </div>
        <div>
          <h1 style="font-size:1.2rem;font-weight:700;color:var(--texto);margin:0;">Reportes</h1>
          <p style="font-size:0.7rem;color:var(--texto-sub);margin:0;">Analiza tus ventas e ingresos en un solo lugar</p>
        </div>
      </div>

      <!-- Selector de fecha como pill -->
      ...
      <div id="rep-resultado">
        <div style="display:flex;align-items:center;gap:0.75rem;color:var(--texto-sub);padding:2rem;justify-content:center;">
          <div class="spinner" style="width:20px;height:20px;"></div> Generando reporte…
        </div>
      </div>
    `;
    generarReporte();
...
async function generarReporte() {
  ...
  try {
    ...
    cont.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:${window.innerWidth<=768?'1fr':'1.4fr 1fr'};gap:1rem;margin-bottom:1.25rem;" class="rep-grid">
        <!-- Ingresos por día -->
        <div class="card">
      ...
      <div style="display:${window.innerWidth<=768?'flex;flex-direction:column':'grid;grid-template-columns:1.4fr 1fr'};gap:1rem;" class="rep-grid">
        <!-- Resumen por tipo de comprobante -->
        <div class="card">
          <div style="font-weight:700;font-size:${window.innerWidth<=768?'0.95':'1'}rem;margin-bottom:1rem;">Resumen por tipo de comprobante</div>
          <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
            <thead><tr style="border-bottom:1px solid var(--gris-borde);">
              <th style="${thCss()}">TIPO</th>
              <th style="${thCss()}">CANTIDAD</th>
              <th style="${thCss()}">TOTAL</th>
              ${window.innerWidth>768?`<th style="${thCss()}"></th>`:''}
            </tr></thead>
            <tbody>
              ${[['Factura','#2563EB',cTipos.factura],['Boleta de venta','#7C3AED',cTipos.boleta]].map(([label,color,datos])=>`
                <tr style="border-bottom:1px solid var(--gris-borde);">
                  <td style="${tdCss()}"><span style="display:inline-flex;align-items:center;gap:0.5rem;"><span style="width:10px;height:10px;border-radius:50%;background:${color};flex-shrink:0;"></span>${label}</span></td>
                  <td style="${tdCss()}">${datos.cnt}</td>
                  <td style="${tdCss()};font-weight:600;">${soles(datos.total)}</td>
                  ${window.innerWidth>768?`<td style="${tdCss()}">
                    <div style="display:flex;align-items:center;gap:0.5rem;">
                      <div style="flex:1;height:8px;background:var(--gris-borde);border-radius:999px;overflow:hidden;">
              ...
              <tr>
                ...
                ${window.innerWidth>768?`<td style="${tdCss()}"><div style="display:flex;align-items:center;gap:0.5rem;"><div style="flex:1;height:8px;background:#2563EB;border-radius:999px;"></div><span style="font-size:0.75rem;color:var(--texto-sub);width:32px;text-align:right;">100%</span></div></td>`:''}
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="card">
          <div style="font-weight:700;font-size:${window.innerWidth<=768?'0.95':'1'}rem;margin-bottom:1rem;">Indicadores adicionales</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
      ...
      ${seccionRest && window.innerWidth<=768 ? `
      <!-- Restaurante PRO — móvil -->
      <div style="margin-top:1rem;">
        <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.85rem;">
          ...
          <span style="font-weight:700;font-size:0.95rem;">Restaurante</span>
          <span style="background:#FFF7ED;color:#EA580C;font-size:0.65rem;font-weight:700;padding:0.15rem 0.55rem;border-radius:999px;border:1px solid #FED7AA;">Plan PRO</span>
        </div>

        <!-- 4 KPIs restaurante 2×2 -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;margin-bottom:1rem;">
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="width:30px;height:30px;border-radius:8px;background:#FFF7ED;display:flex;align-items:center;justify-content:center;margin-bottom:0.4rem;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg>
            </div>
            <div style="font-size:1.1rem;font-weight:700;color:var(--texto);">${soles(totalRest)}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);">Ventas restaurante</div>
            <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;margin-top:0.2rem;"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/></svg>
          </div>
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="width:30px;height:30px;border-radius:8px;background:#FFF7ED;display:flex;align-items:center;justify-content:center;margin-bottom:0.4rem;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><path d="M3 21h18M6 21V7l6-4 6 4v14"/></svg>
            </div>
            <div style="font-size:1.1rem;font-weight:700;color:var(--texto);">${ventasRestCobradas.length}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);">Mesas atendidas</div>
            <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;margin-top:0.2rem;"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/></svg>
          </div>
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="width:30px;height:30px;border-radius:8px;background:#FFF7ED;display:flex;align-items:center;justify-content:center;margin-bottom:0.4rem;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            </div>
            <div style="font-size:1.1rem;font-weight:700;color:var(--texto);">${soles(ticketProm)}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);">Ticket promedio</div>
            <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;margin-top:0.2rem;"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/></svg>
          </div>
          <div class="card" style="padding:0.85rem;min-width:0;">
            <div style="width:30px;height:30px;border-radius:8px;background:#F1F5F9;display:flex;align-items:center;justify-content:center;margin-bottom:0.4rem;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#64748B" stroke-width="2" stroke-linecap="round" style="width:15px;height:15px;"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
            </div>
            <div style="font-size:1.1rem;font-weight:700;color:var(--texto);">${soles(ingresosHotel)}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);">Solo hotel (sin rest.)</div>
            <svg viewBox="0 0 24 24" fill="none" stroke="#64748B" stroke-width="2" stroke-linecap="round" style="width:12px;height:12px;margin-top:0.2rem;"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/></svg>
          </div>
        </div>

        <!-- Platos más vendidos móvil -->
        ${topPlatos.length > 0 ? `
        <div class="card">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.85rem;">
            <div style="display:flex;align-items:center;gap:0.5rem;">
              <span style="font-size:1rem;">🏆</span>
              <span style="font-weight:700;font-size:0.95rem;">Platos más vendidos</span>
            </div>
            <button style="font-size:0.75rem;font-weight:600;color:var(--azul);background:none;border:none;cursor:pointer;display:flex;align-items:center;gap:0.2rem;">
              Ver todos
              <svg viewBox="0 0 24 24" fill="none" stroke="var(--azul)" stroke-width="2.5" stroke-linecap="round" style="width:12px;height:12px;"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
          </div>
          <div style="display:flex;gap:0.75rem;margin-bottom:0.6rem;padding-bottom:0.6rem;border-bottom:1px solid var(--gris-borde);">
            <span style="font-size:0.7rem;font-weight:700;color:var(--texto-sub);width:20px;">#</span>
            <span style="font-size:0.7rem;font-weight:700;color:var(--texto-sub);flex:1;">PLATO / BEBIDA</span>
            <span style="font-size:0.7rem;font-weight:700;color:var(--texto-sub);width:35px;text-align:center;">CANT.</span>
            <span style="font-size:0.7rem;font-weight:700;color:var(--texto-sub);width:60px;text-align:right;">TOTAL</span>
          </div>
          ${topPlatos.map((p,i)=>`
            <div style="display:flex;align-items:center;gap:0.75rem;padding:0.5rem 0;border-bottom:1px solid var(--gris-borde);">
              <span style="font-size:0.85rem;width:20px;">${i===0?'🥇':i===1?'🥈':i===2?'🥉':'#'+(i+1)}</span>
              <span style="font-size:0.82rem;font-weight:600;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(p.nombre)}</span>
              <span style="font-size:0.82rem;width:35px;text-align:center;color:var(--texto-sub);">${p.cantidad}</span>
              <span style="font-size:0.82rem;font-weight:700;color:#16A34A;width:60px;text-align:right;">${soles(p.total)}</span>
            </div>`).join('')}
        </div>` : ''}
      </div>` : (seccionRest||'')}
...
function repKpiMobile(valor, label, color, bg, pct, sparkline, icono) {
  ...
  return `
    <div class="card" style="padding:0.85rem;min-width:0;overflow:hidden;position:relative;">
      <div style="position:absolute;bottom:-6px;right:-6px;opacity:0.12;">${sparkline}</div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.4rem;">
        <div style="width:32px;height:32px;border-radius:9px;background:${bg};display:flex;align-items:center;justify-content:center;">
    ...
    contenido().innerHTML = `
      ...
      <div style="display:flex;align-items:flex-start;gap:0.85rem;margin-bottom:0.85rem;">
        <div style="width:44px;height:44px;border-radius:12px;background:#F0FDF4;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
          <svg viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2" stroke-linecap="round" style="width:22px;height:22px;"><path d="M6 13.87A4 4 0 0 1 7.41 6a5.11 5.11 0 0 1 1.05-1.54 5 5 0 0 1 7.08 0A5.11 5.11 0 0 1 16.59 6 4 4 0 0 1 18 13.87V21H6Z"/><line x1="6" y1="17" x2="18" y2="17"/></svg>
        </div>
        <div>
          <h1 style="font-size:1.2rem;font-weight:700;color:var(--texto);margin:0;">Cocina / Comandas</h1>
          <p style="font-size:0.7rem;color:var(--texto-sub);margin:0.1rem 0 0;">Gestiona los pedidos del restaurante en tiempo real.</p>
        </div>
      </div>

      <!-- Auto-actualiza + hora + botón actualizar -->
      <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">
        <div style="display:flex;align-items:center;gap:0.4rem;font-size:0.75rem;color:var(--texto-sub);">
          <span style="width:8px;height:8px;border-radius:50%;background:#16A34A;"></span>
        ...
        <button onclick="cargarYRenderCocina()" style="margin-left:auto;background:none;border:none;cursor:pointer;color:var(--texto-sub);padding:0.2rem;">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" style="width:18px;height:18px;"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
        </button>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;margin-bottom:1rem;">
        ...
        <div class="card" style="padding:0.9rem;display:flex;align-items:center;gap:0.75rem;min-width:0;">
          <div style="width:36px;height:36px;border-radius:10px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#2563EB" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>
          </div>
          <div style="flex:1;min-width:0;">
            <div style="font-size:1.4rem;font-weight:700;color:var(--texto);line-height:1;">${activas.length}</div>
            <div style="font-size:0.65rem;color:var(--texto-sub);">Total de comandas<br>En proceso</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2.5" stroke-linecap="round" style="width:13px;height:13px;flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>
        </div>
        <!-- Nuevas -->
        <div class="card" style="padding:0.9rem;display:flex;align-items:center;gap:0.75rem;min-width:0;${pendientes.length>0?'background:#FEF2F2;border-color:#FECACA;':''}">
          <div style="width:36px;height:36px;border-radius:10px;background:${pendientes.length>0?'white':'#FEF2F2'};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>
          </div>
          <div style="flex:1;min-width:0;">
            <div style="font-size:1.4rem;font-weight:700;color:#DC2626;line-height:1;">${pendientes.length}</div>
            <div style="font-size:0.72rem;font-weight:700;color:#DC2626;">Nuevas</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2.5" stroke-linecap="round" style="width:13px;height:13px;opacity:0.5;flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>
        </div>
        <!-- Preparando -->
        <div class="card" style="padding:0.9rem;display:flex;align-items:center;gap:0.75rem;min-width:0;${preparando.length>0?'background:#FEFCE8;border-color:#FDE68A;':''}">
          <div style="width:36px;height:36px;border-radius:10px;background:${preparando.length>0?'white':'#FEFCE8'};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#CA8A04" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          </div>
          <div style="flex:1;min-width:0;">
            <div style="font-size:1.4rem;font-weight:700;color:#CA8A04;line-height:1;">${preparando.length}</div>
            <div style="font-size:0.72rem;font-weight:700;color:#CA8A04;">Preparando</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="#CA8A04" stroke-width="2.5" stroke-linecap="round" style="width:13px;height:13px;opacity:0.5;flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>
        </div>
        <!-- Listas -->
        <div class="card" style="padding:0.9rem;display:flex;align-items:center;gap:0.75rem;min-width:0;${listos.length>0?'background:#F0FDF4;border-color:#BBF7D0;':''}">
          <div style="width:36px;height:36px;border-radius:10px;background:${listos.length>0?'white':'#F0FDF4'};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <svg viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          </div>
          <div style="flex:1;min-width:0;">
            <div style="font-size:1.4rem;font-weight:700;color:#16A34A;line-height:1;">${listos.length}</div>
            <div style="font-size:0.72rem;font-weight:700;color:#16A34A;">Listas para servir</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="#16A34A" stroke-width="2.5" stroke-linecap="round" style="width:13px;height:13px;opacity:0.5;flex-shrink:0;"><polyline points="9 18 15 12 9 6"/></svg>
        </div>
      </div>

      <!-- Sección Nuevas -->
      <div style="background:#FEF2F2;border:1.5px solid #FECACA;border-radius:16px;padding:1rem;margin-bottom:0.75rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.85rem;">
          <div style="display:flex;align-items:center;gap:0.65rem;">
            <div style="width:36px;height:36px;border-radius:10px;background:#DC2626;display:flex;align-items:center;justify-content:center;">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>
            </div>
            <div>
              <div style="font-weight:700;font-size:0.95rem;">Nuevas</div>
              <div style="font-size:0.68rem;color:var(--texto-sub);">Pedidos recién recibidos</div>
            </div>
          </div>
          <span style="width:26px;height:26px;border-radius:50%;background:#DC2626;color:white;font-size:0.8rem;font-weight:700;display:flex;align-items:center;justify-content:center;">${pendientes.length}</span>
        </div>
        ${pendientes.length===0
          ? `<div style="background:white;border:2px dashed #FECACA;border-radius:12px;padding:1.75rem;text-align:center;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#FECACA" stroke-width="1.5" stroke-linecap="round" style="width:36px;height:36px;margin:0 auto 0.65rem;">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>
              </svg>
              <div style="font-weight:600;font-size:0.85rem;color:var(--texto);margin-bottom:0.2rem;">No hay comandas nuevas</div>
              <div style="font-size:0.72rem;color:var(--texto-sub);">Cuando lleguen nuevos pedidos se mostrarán aquí.</div>
            </div>`
          : pendientes.map(c=>tarjetaComandaCocinaMobile(c,'nueva')).join('')}
      </div>

      <!-- Sección Preparando -->
      <div style="background:#FEFCE8;border:1.5px solid #FDE68A;border-radius:16px;padding:1rem;margin-bottom:0.75rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.85rem;">
          <div style="display:flex;align-items:center;gap:0.65rem;">
            <div style="width:36px;height:36px;border-radius:10px;background:#CA8A04;display:flex;align-items:center;justify-content:center;">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" style="width:18px;height:18px;"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            </div>
            <div>
              <div style="font-weight:700;font-size:0.95rem;">Preparando</div>
              <div style="font-size:0.68rem;color:var(--texto-sub);">En preparación en cocina</div>
            </div>
          </div>
          <span style="width:26px;height:26px;border-radius:50%;background:#CA8A04;color:white;font-size:0.8rem;font-weight:700;display:flex;align-items:center;justify-content:center;">${preparando.length}</span>
        </div>
        ${preparando.length===0
          ? `<div style="text-align:center;padding:1rem;color:var(--texto-sub);font-size:0.8rem;">Nada en preparación</div>`
          : preparando.map(c=>tarjetaComandaCocinaMobile(c,'preparando')).join('')}
      </div>

      <!-- Sección Listo para servir -->
      <div style="background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:16px;padding:1rem;margin-bottom:0.75rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.85rem;">
          <div style="display:flex;align-items:center;gap:0.65rem;">
            <div style="width:36px;height:36px;border-radius:10px;background:#16A34A;display:flex;align-items:center;justify-content:center;">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" style="width:18px;height:18px;"><polyline points="20 6 9 17 4 12"/></svg>
            </div>
            <div>
              <div style="font-weight:700;font-size:0.95rem;">Listo para servir</div>
              <div style="font-size:0.68rem;color:var(--texto-sub);">Listo para llevar a la mesa</div>
            </div>
          </div>
          <span style="width:26px;height:26px;border-radius:50%;background:#16A34A;color:white;font-size:0.8rem;font-weight:700;display:flex;align-items:center;justify-content:center;">${listos.length}</span>
        </div>
        ${listos.length===0
          ? `<div style="text-align:center;padding:1rem;color:var(--texto-sub);font-size:0.8rem;">Sin platos listos aún</div>`
          : listos.map(c=>tarjetaComandaCocinaMobile(c,'listo')).join('')}
      </div>
    `;
    return;
  }
...
function tarjetaComandaCocinaMobile(comanda, estado) {
  ...
  return `
    <div style="background:white;border:1px solid ${urgente?'#DC2626':'var(--gris-borde)'};border-radius:12px;padding:0.9rem;margin-bottom:0.6rem;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.6rem;">
        <div style="display:flex;align-items:center;gap:0.5rem;">
      ...
      <div style="margin-bottom:0.75rem;">
        ${items.map(it=>`<div style="display:flex;align-items:center;gap:0.5rem;padding:0.2rem 0;">
          <span style="font-size:0.78rem;font-weight:700;color:var(--azul);min-width:20px;">x${it.cantidad}</span>
          <span style="font-size:0.83rem;">${escapeHtml(it.descripcion)}</span>
        </div>`).join('')}
      </div>
      ${estado==='nueva'
        ?`<button onclick="moverComandaCocina('${comanda.id}','preparando')" style="width:100%;padding:0.7rem;background:#EA580C;color:white;border:none;border-radius:10px;font-size:0.83rem;font-weight:700;cursor:pointer;">🔥 Empezar a preparar</button>`
        :estado==='preparando'
        ?`<button onclick="moverComandaCocina('${comanda.id}','listo')" style="width:100%;padding:0.7rem;background:#16A34A;color:white;border:none;border-radius:10px;font-size:0.83rem;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:0.4rem;"><svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" style="width:16px;height:16px;"><polyline points="20 6 9 17 4 12"/></svg>Marcar como listo</button>`
        :`<div style="padding:0.7rem;background:#EFF6FF;border-radius:10px;font-size:0.83rem;font-weight:700;color:#2563EB;text-align:center;">🛎️ Esperando que el mozo lo sirva</div>`}
    </div>`;
}
