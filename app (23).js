...
//  PLAN PRO › RESTAURANTE
// ════════════════════════════════════════════════════════════
let _restCarta = [], _restComandas = [], _restPedidoActual = [], _restMesaActual = null;

async function moduloRestaurante() {
  skeleton();
  try {
    if (!SESSION.turnoActivo) SESSION.turnoActivo = await getTurnoAbierto();
    const { data: carta } = await db.from('productos')
    ...
    _restComandas = (todasComandas||[]).filter(c=>{
      const n = c.notas||'';
      return n.includes('ESTADO:abierta') || n.includes('ESTADO:preparando');
    });
...
function renderRestaurante(mesasOcupadas) {
  ...
  contenido().innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:1.25rem;">
      ...
      <div style="display:flex;gap:0.5rem;">
        <span style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:999px;padding:0.3rem 0.85rem;font-size:0.78rem;font-weight:600;color:#16A34A;">🟢 ${numMesas-mesasOcupadas.size} libres</span>
    ...
    <div style="display:flex;gap:1rem;margin-bottom:1.25rem;font-size:0.8rem;color:var(--texto-sub);">
      <span>🟢 Libre</span><span>🔴 Ocupada</span><span>🟡 Preparando</span>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:1rem;margin-bottom:1.5rem;">
      ${Array.from({length:numMesas},(_,i)=>{
        ...
        const color=enPrep?'#CA8A04':ocupada?'#DC2626':'#16A34A';
        const bg=enPrep?'#FEFCE8':ocupada?'#FEF2F2':'#F0FDF4';
        const label=enPrep?'Preparando':ocupada?'Ocupada':'Libre';
        return `<div onclick="abrirMesaRestaurante(${num})" style="background:white;border:2px solid ${color};border-radius:16px;padding:1.25rem;cursor:pointer;text-align:center;transition:box-shadow 0.15s,transform 0.1s;" onmouseover="this.style.boxShadow='0 6px 20px rgba(0,0,0,0.1)';this.style.transform='translateY(-2px)'" onmouseout="this.style.boxShadow='none';this.style.transform='translateY(0)'">
          <div style="font-size:2rem;margin-bottom:0.35rem;">🍽️</div>
          <div style="font-size:1.25rem;font-weight:800;">Mesa ${num}</div>
          <div style="font-size:0.72rem;font-weight:700;color:${color};background:${bg};border-radius:999px;padding:0.2rem 0.6rem;margin:0.4rem auto 0;display:inline-block;">${label}</div>
          ${ocupada?`<div style="font-size:0.78rem;font-weight:600;color:var(--verde);margin-top:0.4rem;">${soles(total)}</div>`:''}
        </div>`;
      }).join('')}
    </div>
    ${_restComandas.length?`
    <div class="card">
      <div style="font-weight:700;margin-bottom:0.85rem;">📋 Comandas activas (${_restComandas.length})</div>
      ${_restComandas.map(c=>{
        ...
        return `<div style="display:flex;align-items:center;gap:1rem;padding:0.65rem 0;border-bottom:1px solid var(--gris-borde);">
          <span style="font-weight:700;">Mesa ${mesa}</span>
          <span style="flex:1;font-size:0.78rem;color:var(--texto-sub);">${items.length} ítem(s)</span>
          <span style="font-size:0.72rem;font-weight:700;color:${enPrep?'#CA8A04':'#DC2626'};background:${enPrep?'#FEFCE8':'#FEF2F2'};padding:0.2rem 0.6rem;border-radius:999px;">${enPrep?'Preparando':'Abierta'}</span>
          <span style="font-weight:700;color:var(--verde);">${soles(total)}</span>
          <button onclick="abrirMesaRestaurante(${mesa})" style="${ST.btnSec};padding:0.35rem 0.65rem;font-size:0.78rem;">Ver</button>
        </div>`;
      }).join('')}
    </div>`:''}
  `;
...
async function abrirMesaRestaurante(numMesa) {
  ...
  _restPedidoActual = comanda
    ? (comanda.items_venta_directa||[]).map(it=>({ producto_id:it.producto_id, descripcion:it.descripcion, cantidad:it.cantidad, precio:Number(it.precio_unitario), subtotal:Number(it.subtotal||0) }))
    : [];
...
function renderComandaMesa(numMesa, comandaExistente, estadias) {
  const categorias = [...new Set(_restCarta.map(p=>p.categoria||'general'))];
  ...
  contenido().innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:1.25rem;">
      <div style="display:flex;align-items:center;gap:0.75rem;">
        ...
        ${enPrep?'<span style="background:#FEFCE8;color:#CA8A04;font-size:0.75rem;font-weight:700;padding:0.2rem 0.65rem;border-radius:999px;">Preparando en cocina</span>':''}
      </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 320px;gap:1.25rem;align-items:start;">
      ...
      <div>
        <div style="display:flex;gap:0.4rem;flex-wrap:wrap;margin-bottom:1rem;">
          ...
          ${categorias.map(c=>`<button onclick="filtrarCarta('${escapeHtml(c)}',this)" class="cat-carta" style="padding:0.4rem 0.85rem;border-radius:999px;font-size:0.8rem;font-weight:600;background:white;color:var(--texto-sub);border:1.5px solid var(--gris-borde);cursor:pointer;">${escapeHtml(c.charAt(0).toUpperCase()+c.slice(1))}</button>`).join('')}
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(155px,1fr));gap:0.75rem;" id="carta-grid">
          ${_restCarta.length===0
            ? `<div style="grid-column:1/-1;text-align:center;color:var(--texto-sub);padding:2rem;">Sin productos. <a href="#" onclick="navegarA('tiendita');return false;" style="color:var(--azul);font-weight:600;">Agrégalos en Tiendita</a></div>`
            : _restCarta.map(p=>`
              <div class="plato-card" data-cat="${escapeHtml(p.categoria||'general')}" onclick="${p.stock_actual<=0?'toast(\'Sin stock\',\'\',\'warn\')':''};${p.stock_actual>0?`agregarAlPedido('${p.id}','${escapeHtml(p.nombre).replace(/'/g,"\\'")}',${p.precio_venta})`:''}"
                style="background:white;border:1.5px solid var(--gris-borde);border-radius:14px;padding:1rem;cursor:${p.stock_actual<=0?'not-allowed':'pointer'};opacity:${p.stock_actual<=0?'0.5':'1'};transition:border-color 0.15s;"
                onmouseover="${p.stock_actual>0?"this.style.borderColor='var(--azul)';this.style.boxShadow='0 4px 12px rgba(37,99,235,0.15)'":''}"
                onmouseout="${p.stock_actual>0?"this.style.borderColor='var(--gris-borde)';this.style.boxShadow='none'":''}">
                <div style="font-size:1.75rem;text-align:center;margin-bottom:0.4rem;">${p.categoria==='bebidas'?'🥤':p.categoria==='postres'?'🍰':p.categoria==='entradas'?'🥗':'🍽️'}</div>
                <div style="font-weight:600;font-size:0.85rem;text-align:center;margin-bottom:0.25rem;">${escapeHtml(p.nombre)}</div>
                <div style="font-weight:700;color:var(--verde);text-align:center;">${soles(p.precio_venta)}</div>
                ${p.stock_actual<=0?'<div style="font-size:0.68rem;color:var(--rojo);text-align:center;margin-top:0.25rem;">Sin stock</div>':''}
              </div>`).join('')}
        </div>
      </div>
      <!-- Panel pedido -->
  ...
  return _restPedidoActual.map((it,i)=>`
    <div style="display:flex;align-items:center;gap:0.5rem;padding:0.5rem 0;border-bottom:1px solid var(--gris-borde);">
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:0.75rem;color:var(--texto-sub);">${soles(it.precio)} c/u</div>
      </div>
...
function agregarAlPedido(productoId, nombre, precio) {
  const idx = _restPedidoActual.findIndex(i=>i.producto_id===productoId);
  ...
  else { _restPedidoActual.push({ producto_id:productoId, descripcion:nombre, cantidad:1, precio:Number(precio), subtotal:Number(precio) }); }
...
async function enviarComandaCocina(numMesa, comandaId) {
  ...
  try {
    ...
    await db.from('items_venta_directa').insert(_restPedidoActual.map(it=>({
      ...
      producto_id: it.producto_id||null, descripcion: it.descripcion,
...
function abrirCobroMesa(numMesa, comandaId, tipo) {
  ...
  if (tipo==='habitacion') {
    ...
    abrirModal('Cobrar — Mesa '+numMesa, `
      <div style="text-align:center;background:var(--gris-bg);border-radius:12px;padding:1.25rem;margin-bottom:1rem;">
        <div style="font-size:0.78rem;color:var(--texto-sub);">Total a cobrar</div>
        <div style="font-size:1.75rem;font-weight:700;color:var(--verde);">${soles(total)}</div>
      </div>
      <div style="${ST.grupo}"><label style="${ST.label}">Método de pago</label>
        <select style="${ST.input}" id="rest-metodo">
          <option value="efectivo">Efectivo</option><option value="yape">Yape</option>
          <option value="plin">Plin</option><option value="transferencia">Transferencia</option>
        </select>
      </div>
      <button style="${ST.btnPri}" onclick="confirmarCobroMesa('${comandaId}',${numMesa},${total})">Confirmar cobro</button>
    `, { ancho:'380px' });
  }
}
...
async function confirmarCobroMesa(comandaId, numMesa, total) {
  ...
  try {
    ...
    if (comandaId&&comandaId!=='null') {
      ...
      await db.from('ventas_directas').update({ metodo_pago:metodo, notas:(cv?.notas||'').replace('ESTADO:preparando','ESTADO:cobrada').replace('ESTADO:abierta','ESTADO:cobrada') }).eq('id',comandaId);
    }
    cerrarModal(); toast('✅ Cobrado',`Mesa ${numMesa} — ${soles(total)}`, 'ok');
    moduloRestaurante();
  } catch(err) { toast('Error',err.message,'error'); }
}
  ...
  try {
    for (const it of _restPedidoActual) {
      await db.from('consumos_estadia').insert({
        ...
        producto_id: it.producto_id||null, descripcion:`[Rest.] ${it.descripcion}`,
        cantidad: it.cantidad, precio_unitario: it.precio,
    ...
    if (comandaId&&comandaId!=='null') {
      const { data:cv } = await db.from('ventas_directas').select('notas').eq('id',comandaId).single();
      await db.from('ventas_directas').update({ metodo_pago:'mixto', notas:(cv?.notas||'').replace('ESTADO:preparando','ESTADO:cobrada').replace('ESTADO:abierta','ESTADO:cobrada') }).eq('id',comandaId);
    }
...
async function cargarYRenderCocina() {
  ...
  const activas = (comandas||[]).filter(c=>{ const n=c.notas||''; return n.includes('ESTADO:preparando')||n.includes('ESTADO:abierta'); });
  ...
  contenido().innerHTML = `
    ...
    `:`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;" class="rep-grid">
      <div>
    ...
    <div style="background:white;border:1.5px solid ${urgente?'#DC2626':'var(--gris-borde)'};border-radius:14px;padding:1.1rem;${urgente?'box-shadow:0 0 0 3px rgba(220,38,38,0.1);':''}">
      ...
      <div style="margin-bottom:0.85rem;">
        ${items.map(it=>`<div style="display:flex;align-items:center;gap:0.5rem;padding:0.3rem 0;border-bottom:1px solid var(--gris-borde);">
          <span style="font-size:1rem;font-weight:700;color:var(--azul);min-width:22px;">×${it.cantidad}</span>
